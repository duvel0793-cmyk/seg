...
class ResNetFeatureEncoder(nn.Module):
    def __init__(self, backbone: str = "resnet18", in_channels: int = 4, pretrained: bool = True) -> None:
        ...
        if backbone != "resnet18":
            raise ValueError("ResNetFeatureEncoder only supports resnet18.")
        ...
        self.pretrained_load_summary = {
            ...
            "status": "torchvision_imagenet" if pretrained else "disabled",
            "path": "torchvision://resnet18_imagenet1k_v1" if pretrained else "",
            "loaded_keys": [],
            "missing_keys": [],
            "unexpected_keys": [],
            "adapted_keys": ["stem.0.weight"] if in_channels != 3 else [],
