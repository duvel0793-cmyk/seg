...
class PostOnlyDamageClassifier(nn.Module):
    def __init__(
        ...
        self.pool = MaskedMultiScalePooling(feature_channels={"c5": self.encoder.feature_channels["c5"]}, pool_modes=("avg", "max"))
...
class SiameseSimpleDamageClassifier(nn.Module):
    def __init__(
        ...
        self.pool = MaskedMultiScalePooling(feature_channels={"c5": deepest_channels}, pool_modes=("avg", "max"))
        self.classifier = InstanceClassifierHead(
            in_features=deepest_channels * 2,
            hidden_features=max(256, deepest_channels),
            dropout=dropout,
