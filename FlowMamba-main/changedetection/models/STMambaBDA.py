...
class FlowMamba(STMambaBDA):
    ...
    def align(self, pre_features: List[Tensor], post_features: List[Tensor], label: Tensor | None = None):
        ...
        for index, flow_module in enumerate(self.flow):
            ...
            if label is not None:
                mask = F.interpolate(label.float(), size=aligned_features[index].shape[-2:], mode="nearest") == 1
                total_loss = total_loss + self.criterion(pre_features[index] * mask, aligned_features[index] * mask)
