...
class Trainer:
    ...
    def train(self):
        ...
        self.logger.info(
            "Training option: FlowMamba + lovasz + ce + align "
            f"(align_loss_weight={self.args.align_loss_weight})"
        )
        ...
        for iteration, batch in enumerate(self.train_loader, start=1):
            ...
            clf_loss = F.cross_entropy(output_clf, labels_clf, ignore_index=255)
            clf_loss = clf_loss + 0.75 * L.lovasz_softmax(F.softmax(output_clf, dim=1), labels_clf, ignore=255)
            ...
            if iteration % self.args.log_interval == 0:
                ...
                self.logger.info(
                    ...
                    f"bda_loss: {clf_loss.item():.4f} | align_loss: {align_loss.item():.4f} | "
                    f"eta: {eta}"
                )
                start = time.time()
...
def parse_args():
    ...
    parser.add_argument("--pretrained_weight_path", type=str, default=str(PROJECT_ROOT / "pretrained_weight/vssm_small_0229_ckpt_epoch_222.pth"))
