...
def _per_class_f1_aliases(metrics: dict[str, Any]) -> dict[str, float]:
    aliases = {
        "no-damage": "no_damage",
    ...
    return {alias: float(per_class.get(class_name, 0.0)) for class_name, alias in aliases.items()}


def _metrics_with_prefix(metrics: dict[str, Any], prefix: str) -> dict[str, Any]:
    return {
        f"{prefix}_accuracy": float(metrics.get("accuracy", 0.0)),
        f"{prefix}_macro_f1": float(metrics.get("macro_f1", 0.0)),
        f"{prefix}_weighted_f1": float(metrics.get("weighted_f1", 0.0)),
        f"{prefix}_per_class_f1": _per_class_f1(metrics),
        f"{prefix}_per_class_f1_aliases": _per_class_f1_aliases(metrics),
        f"{prefix}_confusion_matrix": metrics.get("confusion_matrix", []),
    }
...
def run_epoch(
    *,
    ...
    all_pixel_instance_predictions: list[int] = []
    all_pixel_instance_probabilities: list[list[float]] = []
    ...
    for batch_idx, batch in enumerate(progress):
        ...
        with _inference_context(not is_train):
        ...
        pixel_instance_probs_tensor = outputs.get("pixel_instance_probabilities")
        pixel_instance_pred_tensor = outputs.get("pixel_instance_pred_labels")
        pixel_mean_probs_tensor = outputs.get("pixel_instance_probabilities_mean")
        pixel_topk_probs_tensor = outputs.get("pixel_instance_probabilities_topk")
        pixel_class_probabilities_tensor = outputs.get("pixel_class_probabilities")
        pixel_pred_tensor = outputs.get("pixel_pred_labels")
        pixel_valid_mask_tensor = outputs.get("pixel_valid_mask")
        ...
        pixel_instance_probabilities = None if pixel_instance_probs_tensor is None else pixel_instance_probs_tensor.detach().float().cpu()
        pixel_instance_predictions = None if pixel_instance_pred_tensor is None else pixel_instance_pred_tensor.detach().cpu().tolist()
        pixel_mean_probabilities = None if pixel_mean_probs_tensor is None else pixel_mean_probs_tensor.detach().float().cpu()
        pixel_topk_probabilities = None if pixel_topk_probs_tensor is None else pixel_topk_probs_tensor.detach().float().cpu()
        pixel_class_probabilities = None if pixel_class_probabilities_tensor is None else pixel_class_probabilities_tensor.detach().float().cpu()
        pixel_predictions = None if pixel_pred_tensor is None else pixel_pred_tensor.detach().cpu()
        pixel_valid_mask = None if pixel_valid_mask_tensor is None else pixel_valid_mask_tensor.detach().float().cpu()
        ...
        if pixel_instance_probabilities is not None:
            all_pixel_instance_probabilities.extend(pixel_instance_probabilities.tolist())
            all_pixel_instance_predictions.extend([] if pixel_instance_predictions is None else pixel_instance_predictions)
                ...
                prediction_records.append(
                    {
                        ...
                        "bbox": meta["bbox"],
                        "tight_bbox_xyxy": meta.get("tight_bbox_xyxy"),
                        "context_bbox_xyxy": meta.get("context_bbox_xyxy"),
                        "neighborhood_bbox_xyxy": meta.get("neighborhood_bbox_xyxy"),
                        "gt_label": int(targets[row_idx]),
                        "pred_label": int(predictions[row_idx]),
                        "instance_pred_label": int(corn_predictions[row_idx]),
                        "corn_pred_label": int(corn_predictions[row_idx]),
                        "final_pred_label": int(predictions[row_idx]),
                        "pixel_agg_pred_label": None if pixel_instance_predictions is None else int(pixel_instance_predictions[row_idx]),
                        ...
                        "instance_probabilities": corn_probabilities[row_idx].tolist(),
                        ...
                        "pixel_instance_probabilities": (
                            None if pixel_instance_probabilities is None else pixel_instance_probabilities[row_idx].tolist()
                        ),
                        "pixel_instance_probabilities_mean": (
                            None if pixel_mean_probabilities is None else pixel_mean_probabilities[row_idx].tolist()
                        ),
                        "pixel_instance_probabilities_topk": (
                            None if pixel_topk_probabilities is None else pixel_topk_probabilities[row_idx].tolist()
                        ),
                        "pixel_feature_source": outputs.get("pixel_feature_source"),
                        "pixel_pred_map": None if pixel_predictions is None else pixel_predictions[row_idx].tolist(),
                        "pixel_valid_mask": None if pixel_valid_mask is None else pixel_valid_mask[row_idx, 0].tolist(),
    ...
    main_instance_metrics = compute_classification_metrics(all_targets, all_corn_predictions, corn_probabilities_np)
    pixel_instance_metrics = None
    if all_pixel_instance_predictions and len(all_pixel_instance_predictions) == len(all_targets):
        pixel_instance_metrics = compute_classification_metrics(
            all_targets,
            all_pixel_instance_predictions,
            np.asarray(all_pixel_instance_probabilities, dtype=np.float64),
        )
    ...
    pixel_fusion_active = bool(config["model"].get("enable_pixel_ordinal_line", False)) and bool(
        config["model"].get("pixel_line_use_for_final_prediction", False)
    ) and float(config["model"].get("pixel_line_prob_mix_weight", 0.0)) > 0.0
    metrics["feature_stats"] = feature_stats
    metrics.update(_metrics_with_prefix(main_instance_metrics, "corn"))
    metrics.update(_metrics_with_prefix(main_instance_metrics, "main_instance"))
    metrics["instance_macro_f1"] = metrics["main_instance_macro_f1"]
    metrics["instance_per_class_f1"] = metrics["main_instance_per_class_f1"]
    metrics.update(_metrics_with_prefix(metrics, "final"))
    metrics["final_prediction_active"] = bool(
        conditional_review_fusion_active
        or structural_fusion_active
        or router_fusion_active
        or scale_aux_fusion_active
        or pixel_fusion_active
    )
    ...
    if pixel_instance_metrics is not None:
        metrics.update(_metrics_with_prefix(pixel_instance_metrics, "pixel_agg"))
        metrics["pixel_inst_accuracy"] = metrics["pixel_agg_accuracy"]
        metrics["pixel_inst_macro_f1"] = metrics["pixel_agg_macro_f1"]
        metrics["pixel_inst_weighted_f1"] = metrics["pixel_agg_weighted_f1"]
        metrics["pixel_inst_per_class_f1"] = metrics["pixel_agg_per_class_f1_aliases"]
        metrics["pixel_inst_confusion_matrix"] = metrics["pixel_agg_confusion_matrix"]
    else:
        metrics["pixel_agg_accuracy"] = None
        metrics["pixel_agg_macro_f1"] = None
        metrics["pixel_agg_weighted_f1"] = None
        metrics["pixel_agg_per_class_f1"] = None
        metrics["pixel_agg_per_class_f1_aliases"] = None
        metrics["pixel_agg_confusion_matrix"] = None
        metrics["pixel_inst_accuracy"] = None
        metrics["pixel_inst_macro_f1"] = None
        metrics["pixel_inst_weighted_f1"] = None
        metrics["pixel_inst_per_class_f1"] = None
        metrics["pixel_inst_confusion_matrix"] = None
    diagnostics_extra = {
        "corn_macro_f1": metrics.get("corn_macro_f1"),
        "corn_per_class_f1": metrics.get("corn_per_class_f1"),
        "main_instance_macro_f1": metrics.get("main_instance_macro_f1"),
        "main_instance_per_class_f1": metrics.get("main_instance_per_class_f1"),
        "pixel_agg_macro_f1": metrics.get("pixel_agg_macro_f1"),
        "pixel_agg_per_class_f1": metrics.get("pixel_agg_per_class_f1"),
