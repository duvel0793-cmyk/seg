...
def build_run_summary(config: dict[str, Any], model: torch.nn.Module, optimizer_groups: list[dict[str, Any]], *, include_model_repr: bool) -> str:
    ...
    lines.extend(
        [
            ...
            f"use_structural_two_stage_head: {bool(config['model'].get('use_structural_two_stage_head', False))}",
            f"use_severity_aware_scale_router: {bool(config['model'].get('use_severity_aware_scale_router', False))}",
            (
...
def main() -> None:
    ...
    for epoch in range(int(config["training"]["epochs"])):
        ...
        if improved:
            ...
            write_json(output_dir / "confusion_matrix_best.json", ema_val_result["metrics"].get("confusion_matrix", []))
            ...
            if ema_val_result.get("prediction_records"):
                ...
                write_prediction_records_csv(output_dir / "predictions_best.csv", ema_val_result["prediction_records"])

        if ema_val_result.get("diagnostics") is not None:
            write_json(output_dir / f"diagnostics_epoch_{epoch + 1:02d}.json", ema_val_result["diagnostics"])
            write_json(output_dir / "diagnostics_last.json", ema_val_result["diagnostics"])
        write_json(output_dir / "confusion_matrix_last.json", ema_val_result["metrics"].get("confusion_matrix", []))
        if ema_val_result.get("threshold_sweep") is not None:
        ...
        if ema_val_result.get("prediction_records"):
            write_prediction_records(output_dir / "predictions_last.jsonl", ema_val_result["prediction_records"])
            write_prediction_records_csv(output_dir / "predictions_last.csv", ema_val_result["prediction_records"])
        ...
        history_record = {
            ...
            "raw_macro_f1": None if raw_val_result is None else raw_val_result["metrics"]["macro_f1"],
            "ema_macro_f1": ema_val_result["metrics"]["macro_f1"],
            "raw_qwk": None if raw_val_result is None else raw_val_result["metrics"]["QWK"],
            "ema_qwk": ema_val_result["metrics"]["QWK"],
            "raw_far_error": None if raw_val_result is None else raw_val_result["metrics"]["far_error"],
            "ema_far_error": ema_val_result["metrics"]["far_error"],
            "no_f1": ema_val_result["metrics"]["no_f1"],
            "minor_f1": ema_val_result["metrics"]["minor_f1"],
            "major_f1": ema_val_result["metrics"]["major_f1"],
            "destroyed_f1": ema_val_result["metrics"]["destroyed_f1"],
            ...
            "gt_major_pred_no_rate": ema_val_result["metrics"]["gt_major_pred_no_rate"],
            "gt_destroyed_pred_major_rate": ema_val_result["metrics"]["gt_destroyed_pred_major_rate"],
            "pred_label_distribution": ema_val_result["metrics"]["prediction_distribution"],
            "gt_label_distribution": ema_val_result["metrics"]["gt_distribution"],
            "scale_router_gates": ema_val_result["metrics"].get("scale_router_gates"),
            "structural_head_macro_f1": ema_val_result["metrics"].get("structural_head_macro_f1"),
            "scale_router_macro_f1": ema_val_result["metrics"].get("scale_router_macro_f1"),
            "final_macro_f1": ema_val_result["metrics"].get("final_macro_f1") if ema_val_result["metrics"].get("final_prediction_active") else None,
        ...
        print(
            ...
            f"ema_qwk={ema_val_result['metrics']['QWK']:.4f} "
            f"ema_far_error={ema_val_result['metrics']['far_error']:.4f} "
            f"no_f1={ema_val_result['metrics']['no_f1']:.4f} "
            f"minor_f1={ema_val_result['metrics']['minor_f1']:.4f} "
            f"major_f1={ema_val_result['metrics']['major_f1']:.4f} "
            f"destroyed_f1={ema_val_result['metrics']['destroyed_f1']:.4f} "
            ...
            f"minor_no_rate={ema_val_result['metrics']['gt_minor_pred_no_rate']:.4f} "
            f"minor_major_rate={ema_val_result['metrics']['gt_minor_pred_major_rate']:.4f} "
            f"major_minor_rate={ema_val_result['metrics']['gt_major_pred_minor_rate']:.4f} "
