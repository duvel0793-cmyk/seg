...
DEFAULT_TRAIN_CONFIG_PATH = Path(__file__).resolve().parent / "configs" / "evidence_clean_v1_next_3scale_scale_aux_fusion_safe.yaml"
...
def apply_runtime_overrides(
    ...
    *,
    ...
    if max_epochs is not None:
        config["training"]["epochs"] = max(1, int(max_epochs))
    return config
...
def configure_train_target(model: torch.nn.Module, config: dict[str, Any]) -> None:
    experiment_cfg = config.get("experiment", {}) if isinstance(config.get("experiment"), dict) else {}
    train_target = str(experiment_cfg.get("train_target", "instance")).lower()
    experiment_mode = str(experiment_cfg.get("mode", "")).lower()
    freeze_backbone = bool(experiment_cfg.get("freeze_backbone", False))
    freeze_instance_head = bool(experiment_cfg.get("freeze_instance_head", False))
    if freeze_backbone:
        _set_module_requires_grad(getattr(model, "backbone", None), False)
    pixel_line_enabled = bool(config["model"].get("enable_pixel_ordinal_line", False)) or (experiment_mode == "pixel_ordinal")
    if train_target == "pixel" and not pixel_line_enabled:
        raise RuntimeError("experiment.train_target='pixel' requires model.enable_pixel_ordinal_line=true.")
    if freeze_instance_head:
        for module_name in [
            "cross_scale",
            "instance_project",
            "calibration",
            ...
            "global_corn_aux",
            "minor_no_aux_head",
            "minor_major_aux_head",
            "structural_two_stage_head",
            "conditional_review_head",
            "severity_aware_scale_router",
            "scale_aux_fusion_head",
        ]:
            _set_module_requires_grad(getattr(model, module_name, None), False)


def _active_loss_weights(config: dict[str, Any]) -> dict[str, float]:
    pixel_loss_cfg = config["loss"].get("pixel_line", {}) if isinstance(config["loss"].get("pixel_line"), dict) else {}
    return {
        ...
        "conditional_review_ce_weight": float(config["loss"].get("conditional_review_ce_weight", 0.0)),
        "conditional_review_low_ce_weight": float(config["loss"].get("conditional_review_low_ce_weight", 0.0)),
        "conditional_review_high_ce_weight": float(config["loss"].get("conditional_review_high_ce_weight", 0.0)),
        "conditional_review_delta_l2_weight": float(config["loss"].get("conditional_review_delta_l2_weight", 0.0)),
        "scale_aux_tight_weight": float(config["loss"].get("scale_aux_tight_weight", 0.0)),
        "scale_aux_context_weight": float(config["loss"].get("scale_aux_context_weight", 0.0)),
        "scale_aux_neighborhood_weight": float(config["loss"].get("scale_aux_neighborhood_weight", 0.0)),
        "scale_aux_fusion_ce_weight": float(config["loss"].get("scale_aux_fusion_ce_weight", 0.0)),
        "scale_aux_weight_entropy_weight": float(config["loss"].get("scale_aux_weight_entropy_weight", 0.0)),
        "scale_aux_weight_prior_kl_weight": float(config["loss"].get("scale_aux_weight_prior_kl_weight", 0.0)),
        "pixel_agg_weight": float(pixel_loss_cfg.get("agg_weight", 0.0)) if bool(pixel_loss_cfg.get("enabled", False)) else 0.0,
        "pixel_dense_weight": float(pixel_loss_cfg.get("dense_weight", 0.0)) if bool(pixel_loss_cfg.get("enabled", False)) else 0.0,
        "pixel_entropy_weight": float(pixel_loss_cfg.get("entropy_weight", 0.0)) if bool(pixel_loss_cfg.get("enabled", False)) else 0.0,
    }
...
def _head_registered(model: torch.nn.Module, attribute_name: str) -> bool:
    return getattr(model, attribute_name, None) is not None


def assert_optional_heads_registered(model: torch.nn.Module, config: dict[str, Any]) -> None:
    model_cfg = config["model"]
    checks = [
        ("use_structural_two_stage_head", "structural_two_stage_head"),
        ("use_conditional_review_head", "conditional_review_head"),
        ("use_severity_aware_scale_router", "severity_aware_scale_router"),
        ("use_scale_aux_fusion_head", "scale_aux_fusion_head"),
    ]
    for config_flag, attribute_name in checks:
        if bool(model_cfg.get(config_flag, False)) and not _head_registered(model, attribute_name):
            raise RuntimeError(
                f"Config enables '{config_flag}', but model.{attribute_name} is not registered. "
                "The running code path does not match the requested architecture."
            )
...
def build_run_summary(config: dict[str, Any], model: torch.nn.Module, optimizer_groups: list[dict[str, Any]], *, include_model_repr: bool) -> str:
    ...
    lines = [
        ...
        f"experiment_mode: {experiment_cfg.get('mode', 'instance')}",
        f"train_target: {experiment_cfg.get('train_target', 'instance')}",
    ...
    lines.extend(
        [
            ...
            f"structural_two_stage_head_registered: {_head_registered(model, 'structural_two_stage_head')}",
            f"use_conditional_review_head: {bool(model_cfg.get('use_conditional_review_head', False))}",
            f"conditional_review_use_for_prediction: {bool(model_cfg.get('conditional_review_use_for_prediction', False))}",
            f"conditional_review_prob_mix_weight: {float(model_cfg.get('conditional_review_prob_mix_weight', 0.0))}",
            f"conditional_review_hidden_dim: {int(model_cfg.get('conditional_review_hidden_dim', 512))}",
            f"conditional_review_alpha_init: {float(model_cfg.get('conditional_review_alpha_init', 0.05)):.2f}",
            f"conditional_review_alpha_max: {float(model_cfg.get('conditional_review_alpha_max', 0.30)):.2f}",
            f"conditional_review_detach_base_probs: {bool(model_cfg.get('conditional_review_detach_base_probs', True))}",
            f"conditional_review_head_registered: {_head_registered(model, 'conditional_review_head')}",
            f"use_severity_aware_scale_router: {bool(model_cfg.get('use_severity_aware_scale_router', False))}",
            f"scale_router_use_for_prediction: {bool(model_cfg.get('scale_router_use_for_prediction', False))}",
            f"scale_router_prob_mix_weight: {float(model_cfg.get('scale_router_prob_mix_weight', 0.0))}",
            f"severity_aware_scale_router_registered: {_head_registered(model, 'severity_aware_scale_router')}",
            f"use_scale_aux_fusion_head: {bool(model_cfg.get('use_scale_aux_fusion_head', False))}",
            f"scale_aux_use_for_prediction: {bool(model_cfg.get('scale_aux_use_for_prediction', False))}",
            f"scale_aux_hidden_dim: {int(model_cfg.get('scale_aux_hidden_dim', 512))}",
            f"scale_aux_fusion_mode: {model_cfg.get('scale_aux_fusion_mode', 'dynamic')}",
            f"scale_aux_initial_weights: {list(model_cfg.get('scale_aux_initial_weights', [0.85, 0.06, 0.06, 0.03]))}",
            f"scale_aux_temperature: {float(model_cfg.get('scale_aux_temperature', 1.0))}",
            f"scale_aux_detach_base_probs_for_gate: {bool(model_cfg.get('scale_aux_detach_base_probs_for_gate', True))}",
            f"scale_aux_gate_warmup_epochs: {int(model_cfg.get('scale_aux_gate_warmup_epochs', 0))}",
            f"scale_aux_use_prior_during_warmup: {bool(model_cfg.get('scale_aux_use_prior_during_warmup', False))}",
            f"enable_pixel_ordinal_line: {bool(model_cfg.get('enable_pixel_ordinal_line', False))}",
            f"pixel_line_feature_source: {model_cfg.get('pixel_line_feature_source', 'tight')}",
            f"pixel_line_head_type: {model_cfg.get('pixel_line_head_type', 'two_stage_ordinal')}",
            f"pixel_line_aggregation: {model_cfg.get('pixel_line_aggregation', 'mean_topk_mix')}",
            f"pixel_line_topk_ratio: {float(model_cfg.get('pixel_line_topk_ratio', 0.2))}",
            f"pixel_line_use_for_final_prediction: {bool(model_cfg.get('pixel_line_use_for_final_prediction', False))}",
            f"pixel_line_prob_mix_weight: {float(model_cfg.get('pixel_line_prob_mix_weight', 0.0))}",
            f"scale_aux_fusion_head_registered: {_head_registered(model, 'scale_aux_fusion_head')}",
            (
    ...
    if active_losses:
        for name, value in sorted(active_losses.items()):
            lines.append(f"  {name}: {_format_summary_float(value)}")
    else:
        lines.append("  none")
...
def write_prediction_records_csv(path: Path, records: list[dict[str, Any]]) -> None:
    ...
    fieldnames = [
        ...
        "instance_prob_minor",
        "instance_prob_major",
        "instance_prob_destroyed",
        "pixel_prob_no",
        "pixel_prob_minor",
        "pixel_prob_major",
        "pixel_prob_destroyed",
        "prob_no",
        "prob_minor",
        "prob_major",
        "prob_destroyed",
...
def main() -> None:
    ...
    config = apply_runtime_overrides(config, max_epochs=args.max_epochs)
    ...
    assert_optional_heads_registered(model, config)
    configure_train_target(model, config)
    ...
    for epoch in range(int(config["training"]["epochs"])):
        ...
        history_record = {
            ...
            "conditional_review_ce_loss": train_result["loss_terms"].get("loss_conditional_review_ce"),
            "conditional_review_low_ce_loss": train_result["loss_terms"].get("loss_conditional_review_low_ce"),
            "conditional_review_high_ce_loss": train_result["loss_terms"].get("loss_conditional_review_high_ce"),
            "conditional_review_delta_l2_loss": train_result["loss_terms"].get("loss_conditional_review_delta_l2"),
            "scale_aux_tight_loss": train_result["loss_terms"].get("loss_scale_aux_tight"),
            "scale_aux_context_loss": train_result["loss_terms"].get("loss_scale_aux_context"),
            "scale_aux_neighborhood_loss": train_result["loss_terms"].get("loss_scale_aux_neighborhood"),
            "scale_aux_fusion_ce_loss": train_result["loss_terms"].get("loss_scale_aux_fusion_ce"),
            "scale_aux_weight_entropy_loss": train_result["loss_terms"].get("loss_scale_aux_weight_entropy"),
            "scale_aux_weight_prior_kl_loss": train_result["loss_terms"].get("loss_scale_aux_weight_prior_kl"),
            ...
            "conditional_review_macro_f1": primary_metrics.get("conditional_review_macro_f1"),
            "scale_router_macro_f1": primary_metrics.get("scale_router_macro_f1"),
            "scale_aux_tight_macro_f1": primary_metrics.get("scale_aux_tight_macro_f1"),
            "scale_aux_context_macro_f1": primary_metrics.get("scale_aux_context_macro_f1"),
            "scale_aux_neighborhood_macro_f1": primary_metrics.get("scale_aux_neighborhood_macro_f1"),
            "scale_aux_fused_macro_f1": primary_metrics.get("scale_aux_fused_macro_f1"),
            "main_instance_macro_f1": primary_metrics.get("main_instance_macro_f1"),
            "pixel_agg_macro_f1": primary_metrics.get("pixel_agg_macro_f1"),
            "final_macro_f1": primary_metrics.get("final_macro_f1"),
            ...
            "loss_conditional_review_ce": train_result["loss_terms"].get("loss_conditional_review_ce"),
            "loss_conditional_review_low_ce": train_result["loss_terms"].get("loss_conditional_review_low_ce"),
            "loss_conditional_review_high_ce": train_result["loss_terms"].get("loss_conditional_review_high_ce"),
            "loss_conditional_review_delta_l2": train_result["loss_terms"].get("loss_conditional_review_delta_l2"),
            "loss_scale_aux_tight": train_result["loss_terms"].get("loss_scale_aux_tight"),
            "loss_scale_aux_context": train_result["loss_terms"].get("loss_scale_aux_context"),
            "loss_scale_aux_neighborhood": train_result["loss_terms"].get("loss_scale_aux_neighborhood"),
            "loss_scale_aux_fusion_ce": train_result["loss_terms"].get("loss_scale_aux_fusion_ce"),
            "loss_scale_aux_weight_entropy": train_result["loss_terms"].get("loss_scale_aux_weight_entropy"),
            "loss_scale_aux_weight_prior_kl": train_result["loss_terms"].get("loss_scale_aux_weight_prior_kl"),
            "loss_pixel_agg": train_result["loss_terms"].get("loss_pixel_agg"),
            "loss_pixel_dense": train_result["loss_terms"].get("loss_pixel_dense"),
            "loss_pixel_binary_dense": train_result["loss_terms"].get("loss_pixel_binary_dense"),
            "loss_pixel_severity_dense": train_result["loss_terms"].get("loss_pixel_severity_dense"),
            "loss_pixel_entropy": train_result["loss_terms"].get("loss_pixel_entropy"),
            ...
            "conditional_review_low_gate_mean": train_result["logging_scalars"].get("conditional_review_low_gate_mean"),
            "conditional_review_high_gate_mean": train_result["logging_scalars"].get("conditional_review_high_gate_mean"),
            "conditional_review_alpha_low": train_result["logging_scalars"].get("conditional_review_alpha_low"),
            "conditional_review_alpha_high": train_result["logging_scalars"].get("conditional_review_alpha_high"),
            "scale_aux_weight_base_mean": train_result["logging_scalars"].get("scale_aux_weight_base_mean"),
            "scale_aux_weight_tight_mean": train_result["logging_scalars"].get("scale_aux_weight_tight_mean"),
            "scale_aux_weight_context_mean": train_result["logging_scalars"].get("scale_aux_weight_context_mean"),
            "scale_aux_weight_neighborhood_mean": train_result["logging_scalars"].get("scale_aux_weight_neighborhood_mean"),
            "scale_aux_weight_entropy_mean": train_result["logging_scalars"].get("scale_aux_weight_entropy"),
            "scale_aux_weight_prior_kl_mean": train_result["logging_scalars"].get("scale_aux_weight_prior_kl_mean"),
        ...
        if raw_metrics is not None:
            log_parts.extend(
                [
                    ...
                    f"main_inst_macro_f1={raw_metrics.get('main_instance_macro_f1', raw_metrics['macro_f1']):.4f}",
                    f"pixel_agg_macro_f1={0.0 if raw_metrics.get('pixel_agg_macro_f1') is None else raw_metrics.get('pixel_agg_macro_f1'):.4f}",
                ]
            )
        if ema_metrics is not None:
            log_parts.extend(
                [
                    ...
                    f"ema_pixel_agg_macro_f1={0.0 if ema_metrics.get('pixel_agg_macro_f1') is None else ema_metrics.get('pixel_agg_macro_f1'):.4f}",
                ]
            )
        log_parts.extend(
            [
                ...
                f"review_ce_loss={train_result['loss_terms'].get('loss_conditional_review_ce', 0.0):.4f}",
                f"router_ce_loss={train_result['loss_terms'].get('loss_scale_router_ce', 0.0):.4f}",
                f"pixel_agg_loss={train_result['loss_terms'].get('loss_pixel_agg', 0.0):.4f}",
                f"pixel_dense_loss={train_result['loss_terms'].get('loss_pixel_dense', 0.0):.4f}",
                ...
                f"review_alpha_low={train_result['logging_scalars'].get('conditional_review_alpha_low', 0.0):.4f}",
