...
"""Structural Two-Stage CORN / Hierarchical Structural CORN head."""
...
class StructuralTwoStageHead(nn.Module):
    """Two-Stage CORN / Hierarchical Structural CORN auxiliary head.

    The 4-way ordinal label is factorized into three conditional ordinal
    thresholds:

    - `structural_binary_logit`: ordinal threshold for `y >= 2`
      (`no/minor` vs `major/destroyed`)
    - `low_stage_logit`: ordinal threshold for `y >= 1` inside `{no, minor}`
    - `high_stage_logit`: ordinal threshold for `y >= 3` inside
      `{major, destroyed}`

    Probabilities are combined as:

    - `P(no)        = (1 - p_high) * (1 - p_minor_given_low)`
    - `P(minor)     = (1 - p_high) * p_minor_given_low`
    - `P(major)     = p_high * (1 - p_destroyed_given_high)`
    - `P(destroyed) = p_high * p_destroyed_given_high`
    """
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        # Two-Stage CORN threshold: y >= 2, namely {major, destroyed}.
        structural_binary_logit = self.structural_binary(hidden).squeeze(1)
        # Group-local ordinal threshold: y >= 1 inside {no, minor}.
        low_stage_logit = self.low_stage(hidden).squeeze(1)
        # Group-local ordinal threshold: y >= 3 inside {major, destroyed}.
