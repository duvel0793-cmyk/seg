...
from models.damage_evidence_modules import DamageBDFMLite, EDQALite
...
class ScaleBranch(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.change_block_type = self._resolve_change_block_type(
            change_block_type=change_block_type,
            enable_damage_bdfm_lite=enable_damage_bdfm_lite,
            enable_damage_aware_block=enable_damage_aware_block,
        )
        self.enable_damage_aware_block = self.change_block_type != "none"
        self.enable_change_gate = bool(enable_change_gate) and self.enable_damage_aware_block
        edqa_scale_names = {str(name) for name in edqa_lite_scales}
        self.enable_edqa_lite = bool(enable_edqa_lite) and (self.scale_name in edqa_scale_names)
        ...
        if self.change_block_type == "legacy":
            self.change_block: nn.Module | None = DamageAwareChangeBlock(feature_dim)
        elif self.change_block_type == "bdfm_lite":
            self.change_block = DamageBDFMLite(feature_dim)
        else:
        ...
        self.edqa_lite = (
            EDQALite(
                feature_dim,
                window_size=int(edqa_lite_window_size or window_size),
                num_heads=int(edqa_lite_heads or local_attention_heads),
                dropout=dropout,
                init_alpha=float(edqa_lite_init_alpha),
            )
            if self.enable_edqa_lite
            else None
        )
    ...
    def _resolve_change_block_type(
        self,
        *,
        change_block_type: str,
        enable_damage_bdfm_lite: bool,
        enable_damage_aware_block: bool,
    ) -> str:
        block_type = str(change_block_type).lower()
        if block_type not in {"auto", "none", "legacy", "bdfm_lite"}:
            raise ValueError(f"Unsupported change_block_type='{change_block_type}'")
        if block_type == "auto":
            if bool(enable_damage_bdfm_lite):
                return "bdfm_lite"
            if bool(enable_damage_aware_block):
                return "legacy"
            return "none"
        return block_type
    ...
    def forward(self, *, backbone: nn.Module, pre_input: torch.Tensor, post_input: torch.Tensor, mask: torch.Tensor) -> dict[str, Any]:
        ...
        edqa_outputs: dict[str, torch.Tensor] | None = None
        if self.edqa_lite is not None:
            edqa_outputs = self.edqa_lite(
                fused_feature,
                change_outputs["change_feature"],
                change_outputs["feat_pre_refined"],
                change_outputs["feat_post_refined"],
                mask,
            )
            fused_feature = edqa_outputs["refined_feature"]
        ...
        if edqa_outputs is not None:
            edqa_enabled = refined_feature.new_ones(refined_feature.size(0))
            edqa_alpha = edqa_outputs["edqa_alpha"].to(device=refined_feature.device, dtype=refined_feature.dtype)
            edqa_calibrated_norm = edqa_outputs["edqa_calibrated_norm"].to(device=refined_feature.device, dtype=refined_feature.dtype)
        else:
            edqa_enabled = refined_feature.new_zeros(refined_feature.size(0))
            edqa_alpha = refined_feature.new_zeros(refined_feature.size(0))
            edqa_calibrated_norm = refined_feature.new_zeros(refined_feature.size(0))
        diagnostics = {
            ...
            f"{self.scale_name}_edqa_enabled": edqa_enabled,
            f"{self.scale_name}_edqa_alpha": edqa_alpha,
            f"{self.scale_name}_edqa_calibrated_norm": edqa_calibrated_norm,
        }
        return {
