...
class EvidenceHierarchicalDamageModel(nn.Module):
    ...
    def _combine_prediction_probabilities(
        ...
        mix_terms: list[tuple[torch.Tensor, float]] = []
        structural_weight = self.structural_two_stage_prob_mix_weight if structural_probabilities is not None else 0.0
        router_weight = self.scale_router_prob_mix_weight if scale_router_probabilities is not None else 0.0
        if structural_weight > 0.0 or router_weight > 0.0:
            base_weight = max(0.0, 1.0 - structural_weight - router_weight)
            mix_terms.append((corn_probabilities, base_weight))
            if structural_probabilities is not None and structural_weight > 0.0:
                mix_terms.append((structural_probabilities, structural_weight))
            if scale_router_probabilities is not None and router_weight > 0.0:
                mix_terms.append((scale_router_probabilities, router_weight))
            total_weight = sum(weight for _, weight in mix_terms)
            if total_weight <= 0.0:
                final_probabilities = corn_probabilities
            else:
                final_probabilities = sum(probabilities * weight for probabilities, weight in mix_terms) / total_weight
        else:
            override_probabilities = []
            if structural_probabilities is not None and self.structural_two_stage_use_for_prediction:
                override_probabilities.append(structural_probabilities)
            if scale_router_probabilities is not None and self.scale_router_use_for_prediction:
                override_probabilities.append(scale_router_probabilities)
            final_probabilities = (
                torch.stack(override_probabilities, dim=0).mean(dim=0)
                if override_probabilities
                else corn_probabilities
            )
