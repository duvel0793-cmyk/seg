...
from utils.misc import ensure_dir, get_split_list, load_config, merge_config, resolve_device, seed_worker, set_seed, write_json, write_text, write_yaml
...
def main() -> None:
    ...
    if incompatible.missing_keys or incompatible.unexpected_keys:
        raise RuntimeError(
            f"Checkpoint incompatible. missing={list(incompatible.missing_keys)} unexpected={list(incompatible.unexpected_keys)}"
        )
    ...
    if args.mode in {"instance", "export"}:
        ...
        for record in result["prediction_records"]:
            from utils.misc import append_jsonl
