...
def default_config() -> dict[str, Any]:
    return {
        ...
        "model": {
            ...
            "edqa_lite_window_size": None,
            "edqa_lite_heads": None,
            ...
            "use_structural_two_stage_head": False,
            ...
            "conditional_review_dropout": 0.1,
            ...
            "use_scale_aux_fusion_head": False,
            "scale_aux_use_for_prediction": False,
            "scale_aux_hidden_dim": 512,
            "scale_aux_dropout": 0.1,
            "scale_aux_fusion_mode": "dynamic",
            "scale_aux_initial_weights": [0.85, 0.06, 0.06, 0.03],
            "scale_aux_temperature": 1.0,
            "scale_aux_detach_base_probs_for_gate": True,
            "scale_aux_gate_warmup_epochs": 0,
            "scale_aux_use_prior_during_warmup": False,
            "enable_pixel_ordinal_line": False,
            "pixel_line_feature_source": "tight",
            "pixel_line_head_type": "two_stage_ordinal",
            "pixel_line_hidden_dim": 256,
            "pixel_line_dropout": 0.1,
            "pixel_line_aggregation": "mean_topk_mix",
            "pixel_line_topk_ratio": 0.2,
            "pixel_line_mean_weight": 0.7,
            "pixel_line_topk_weight": 0.3,
            "pixel_line_use_for_final_prediction": False,
            "pixel_line_prob_mix_weight": 0.0,
        },
        "loss": {
            "corn_weight": 0.0,
            ...
            "structural_binary_weight": 0.0,
            "low_stage_weight": 0.0,
            "high_stage_weight": 0.0,
            "scale_router_ce_weight": 0.0,
            "scale_aux_tight_weight": 0.0,
            "scale_aux_context_weight": 0.0,
            "scale_aux_neighborhood_weight": 0.0,
            "scale_aux_fusion_ce_weight": 0.0,
            "scale_aux_weight_entropy_weight": 0.0,
            "scale_aux_weight_prior_kl_weight": 0.0,
            "scale_aux_weight_prior": [0.85, 0.06, 0.06, 0.03],
            ...
            "pixel_line": {
                ...
                "severity_weight": 1.0,
                "binary_weight": 1.0,
                "ignore_mask_outside": True,
                "normalize_by_mask_area": True,
            },
        },
        "experiment": {
            "mode": "instance",
            "train_target": "instance",
            "freeze_instance_head": False,
        ...
        "bridge": {
            ...
            "save_visuals": True,
            "save_official_style": True,
            "aggregate_mode": "global",
            "ignore_background_for_damage": True,
            "eps": 1.0e-8,
        },
    }
...
def _apply_modern_schema_overrides(config: dict[str, Any], raw: dict[str, Any]) -> dict[str, Any]:
    ...
    for field_name in (
        ...
        "fusion_mode",
        "change_block_type",
        "enable_damage_bdfm_lite",
        "enable_damage_aware_block",
        "enable_change_gate",
        "enable_edqa_lite",
        "edqa_lite_scales",
        "edqa_lite_window_size",
        "edqa_lite_heads",
        "edqa_lite_init_alpha",
        "head_type",
        ...
        "use_global_corn_aux",
        "use_structural_two_stage_head",
        "structural_two_stage_hidden_dim",
        "structural_two_stage_dropout",
        "structural_two_stage_use_for_prediction",
        "structural_two_stage_prob_mix_weight",
        "use_conditional_review_head",
        "conditional_review_use_for_prediction",
        "conditional_review_prob_mix_weight",
        "conditional_review_hidden_dim",
        "conditional_review_dropout",
        "conditional_review_alpha_init",
        "conditional_review_alpha_max",
        "conditional_review_detach_base_probs",
        "use_severity_aware_scale_router",
        "scale_router_hidden_dim",
        "scale_router_dropout",
        "scale_router_use_for_prediction",
        "scale_router_prob_mix_weight",
        "scale_router_init_gates",
    ...
    for field_name in (
        ...
        "conditional_review_ce_weight",
        "conditional_review_low_ce_weight",
        "conditional_review_high_ce_weight",
        "conditional_review_delta_l2_weight",
        "conditional_review_low_focus_weight",
        "conditional_review_high_focus_weight",
    ):
...
def _expand_probability_prefix(row: dict[str, Any], prefix: str, probabilities: list[float] | tuple[float, ...] | None) -> None:
    probs = list(probabilities) if probabilities is not None else []
    suffixes = ["no", "minor", "major", "destroyed"]
    for idx, suffix in enumerate(suffixes):
        row[f"{prefix}{suffix}"] = "" if idx >= len(probs) else float(probs[idx])


def prediction_record_to_table_row(record: dict[str, Any]) -> dict[str, Any]:
    row: dict[str, Any] = {
        "sample_id": record.get("sample_id", record.get("sample_index", "")),
        "tile_id": record.get("tile_id", record.get("image_id", "")),
        "building_id": record.get("building_id", record.get("building_idx", "")),
        "gt_label": record.get("gt_label", ""),
        "instance_pred": record.get("instance_pred_label", record.get("corn_pred_label", "")),
        "corn_pred": record.get("corn_pred_label", record.get("pred_label", "")),
        "pixel_agg_pred": record.get("pixel_agg_pred_label", ""),
        "final_pred": record.get("final_pred_label", record.get("pred_label", "")),
        "structural_pred": record.get("structural_pred_label", ""),
        "scale_router_pred": record.get("scale_router_pred_label", ""),
    }
    _expand_probability_prefix(row, "instance_prob_", record.get("instance_probabilities", record.get("corn_class_probabilities")))
    _expand_probability_prefix(row, "pixel_prob_", record.get("pixel_instance_probabilities"))
    _expand_probability_prefix(
        row,
        "prob_",
        record.get("final_class_probabilities", record.get("class_probabilities")),
    )
    _expand_probability_prefix(row, "corn_prob_", record.get("corn_class_probabilities"))
    _expand_probability_prefix(row, "struct_prob_", record.get("structural_class_probabilities"))
    _expand_probability_prefix(row, "router_prob_", record.get("scale_router_probabilities"))
    return row
