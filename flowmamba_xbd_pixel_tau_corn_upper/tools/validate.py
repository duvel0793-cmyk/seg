...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate the standalone VMamba xBD pixel tau CORN upper project.")
    ...
    resolved_config_path = log_dir / "validate_resolved_config.yaml"
    dump_config(cfg, resolved_config_path)
    logger = setup_logger("validate", log_dir / "validate.log")
    logger.info("======== Validate startup ========")
    ...
    state = load_checkpoint(
        ...
        expected_metadata=build_checkpoint_metadata(cfg, model, resolved_config_path=resolved_config_path),
