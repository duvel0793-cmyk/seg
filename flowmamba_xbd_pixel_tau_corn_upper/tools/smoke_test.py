...
from src.utils import ensure_dir, load_config, resolve_device, save_debug_visualization, seed_everything, setup_logger
from src.utils.misc import move_batch_to_device


def main() -> None:
    parser = argparse.ArgumentParser(description="Smoke test for the standalone xBD project.")
    ...
    logger.info(
        "Smoke test backbone=%s reason=%s",
        model.backbone.get_metadata()["backend_name"],
        model.backbone.get_metadata()["backend_reason"],
    )
    logger.info("Smoke test device=%s", device)
