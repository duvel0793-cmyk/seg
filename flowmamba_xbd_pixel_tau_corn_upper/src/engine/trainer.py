...
class Trainer:
    ...
    def train(self) -> None:
        ...
        save_freq = int(max(train_cfg.get("save_freq", self.cfg.get("save_freq", 0)), 0))
        ...
        for epoch in range(start_epoch, int(train_cfg["epochs"])):
            ...
            for batch_idx, batch in progress:
                ...
                progress.set_postfix(
                    total=f"{meters['total_loss'].avg:.4f}",
                    loc=f"{meters['loc_loss'].avg:.4f}",
                    corn=f"{meters['pixel_corn_main'].avg:.4f}",
                    inst=f"{meters['instance_corn_aux'].avg:.4f}",
                )
