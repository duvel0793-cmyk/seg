...
class HungarianMatcher:
    def __init__(
        ...
        *,
        ...
        self._warned_target_truncation = False

    @staticmethod
    def _select_target_subset(target: dict[str, torch.Tensor], max_targets: int) -> tuple[dict[str, torch.Tensor], torch.Tensor]:
        labels = target["labels"]
        num_targets = int(labels.numel())
        if num_targets <= max_targets:
            keep_indices = torch.arange(num_targets, device=labels.device, dtype=torch.long)
            return target, keep_indices

        if "masks" in target and target["masks"].numel() > 0:
            scores = target["masks"].flatten(1).sum(dim=1)
        else:
            boxes = target["boxes_norm"]
            scores = boxes[:, 2] * boxes[:, 3]
        keep_indices = torch.topk(scores, k=max_targets, largest=True, sorted=False).indices

        subset: dict[str, torch.Tensor] = {}
        for key, value in target.items():
            if torch.is_tensor(value) and value.ndim > 0 and value.shape[0] == num_targets:
                subset[key] = value.index_select(0, keep_indices)
            else:
                subset[key] = value
        return subset, keep_indices
    ...
    def __call__(self, outputs: dict[str, Any], targets: list[dict[str, torch.Tensor]]) -> list[tuple[torch.Tensor, torch.Tensor]]:
        ...
        num_queries = int(outputs["pred_logits"].shape[1])
        for batch_index, target in enumerate(targets):
            ...
            if num_targets > num_queries:
                matched_target, keep_indices = self._select_target_subset(target, num_queries)
                if not self._warned_target_truncation:
                    warnings.warn(
                        f"Target count ({num_targets}) exceeds query count ({num_queries}); "
                        f"matching only the largest {num_queries} instances per image.",
                        stacklevel=2,
                    )
                    self._warned_target_truncation = True

            cost = self._build_cost_matrix(outputs, matched_target, batch_index).detach().cpu().numpy()
            pred_indices, target_indices = hungarian_assignment(cost)
            if keep_indices is not None and target_indices.size > 0:
                target_indices = keep_indices[target_indices].detach().cpu().numpy()
            matches.append(
                (
