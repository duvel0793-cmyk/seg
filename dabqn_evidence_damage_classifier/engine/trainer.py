...
def _validation_mode(
    *,
    ...
    if epoch_number == int(total_epochs):
        return "full"
    eval_interval = max(1, int(training_cfg.get("eval_interval", 1)))
    if epoch_number % eval_interval != 0:
        return None
    if epoch_number < int(training_cfg.get("skip_full_eval_until_epoch", 0)):
        return "fast"
    full_eval_interval = max(1, int(training_cfg.get("full_eval_interval", 1)))
    if epoch_number % full_eval_interval == 0:
        return "full"
    return "fast"
    ...
    *,
    ...
    for batch_index, batch in batch_iterator:
        ...
        with _autocast_context(device, bool(config["training"].get("amp", True)), str(config["training"].get("amp_dtype", "bf16"))):
            outputs = model.forward_localization(
                batch,
                stage=stage,
                compute_post=None,
                return_full_masks=bool(config["model"].get("return_full_masks_train", True)),
            )
...
def run_training(config: dict[str, Any]) -> dict[str, Any]:
    ...
    logger.info(
        "stage=%s pre_only_localization=%s post_backbone_skipped=%s forward_damage_skipped=%s mask_loss_source=%s return_full_masks_train=%s",
        stage,
        bool(config["model"].get("localization_pre_only", False)),
        bool(str(stage) == "localization" and config["model"].get("localization_pre_only", False)),
        bool(str(stage) == "localization"),
        str(config.get("loss", {}).get("mask_loss_source", "lowres" if stage == "localization" else "fullres")),
        bool(config["model"].get("return_full_masks_train", True)),
    )
    logger.info(
        "validation_schedule eval_interval=%d full_eval_interval=%d fast_val_max_images=%d skip_full_eval_until_epoch=%d need_damage_metrics=%s",
        int(training_cfg.get("eval_interval", 1)),
        int(training_cfg.get("full_eval_interval", 1)),
        int(training_cfg.get("fast_val_max_images", 30)),
        int(training_cfg.get("skip_full_eval_until_epoch", 0)),
        bool(eval_cfg.get("need_damage_metrics", True)),
    )
    ...
    for epoch in range(int(config["training"].get("epochs", 20))):
        ...
        if validation_mode is None:
            ...
            logger.info("skipping validation for epoch=%d", epoch + 1)
            continue
        fast_val_max_images = int(training_cfg.get("fast_val_max_images", 30))
        max_images = None if validation_mode == "full" else fast_val_max_images
        logger.info("running %s validation for epoch=%d max_images=%s", validation_mode, epoch + 1, "all" if max_images is None else max_images)
        ...
        append_jsonl(history_path, record)
        logger.info(
            "epoch=%d mode=%s train_time=%.1fs val_time=%.1fs train_loss=%.4f avg_gt=%.1f p50_gt=%.1f p90_gt=%.1f p95_gt=%.1f max_gt=%.0f gt_over_query=%.0f gt_over_patch_cap=%.0f adaptive_shrink=%.0f fallback_keep=%.0f empty_patch=%.0f positive_patch=%.0f before_nms=%d after_nms=%d loc_f1=%.4f matched_macro_f1=%.4f e2e_macro_f1=%.4f bridge=%.4f",
            epoch + 1,
            validation_mode,
            train_time_sec,
            val_time_sec,
