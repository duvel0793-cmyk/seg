...
class DABQNLoss(nn.Module):
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        default_mask_source = "lowres" if str(config.get("training", {}).get("stage", "joint")) == "localization" else "fullres"
        self.mask_loss_source = str(loss_cfg.get("mask_loss_source", default_mask_source))
    ...
    def _loss_masks(
        ...
        bce_losses = []
        dice_losses = []
        if self.mask_loss_source == "lowres":
            mask_tensor = outputs["pred_masks_lowres"]
        elif self.mask_loss_source == "fullres":
            mask_tensor = outputs.get("pred_masks")
            if mask_tensor is None:
                raise RuntimeError("loss.mask_loss_source='fullres' requires outputs['pred_masks'].")
        else:
            raise ValueError(f"Unsupported loss.mask_loss_source='{self.mask_loss_source}'.")
        for batch_index, (pred_indices, target_indices) in enumerate(matches):
            if pred_indices.numel() == 0:
                continue
            pred_masks = mask_tensor[batch_index, pred_indices]
        ...
        zero = mask_tensor.new_tensor(0.0)
