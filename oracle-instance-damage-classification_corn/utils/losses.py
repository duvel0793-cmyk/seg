...
class DamageLossModule(nn.Module):
    ...
    def forward(
        ...
        corn_mode = self.loss_mode if self.loss_mode in {"corn", "corn_adaptive_tau_safe"} else None
        ...
        if aux_logits is not None:
            ...
            loss_backward = loss_backward + self.lambda_aux * loss_aux
        ...
        state: dict[str, Any] = {
            ...
            "tau_regularizer": {
            ...
            } if self.loss_mode == "adaptive_ucl_cda_v2" else (
                {
                    ...
                    "lambda_raw_tau_center": self.lambda_raw_tau_center,
                    "lambda_raw_tau_bound": self.lambda_raw_tau_bound,
                    "lambda_corn_soft": self.lambda_corn_soft if self.loss_mode == "corn_adaptive_tau_safe" else None,
                    "enable_corn_soft_emd": self.enable_corn_soft_emd if self.loss_mode == "corn_adaptive_tau_safe" else None,
                    "lambda_corn_soft_emd": self.lambda_corn_soft_emd if self.loss_mode == "corn_adaptive_tau_safe" else None,
                    "enable_decoded_unimodal_reg": self.enable_decoded_unimodal_reg if self.loss_mode == "corn_adaptive_tau_safe" else None,
                    "lambda_decoded_unimodal": self.lambda_decoded_unimodal if self.loss_mode == "corn_adaptive_tau_safe" else None,
                    "decoded_unimodal_margin": self.decoded_unimodal_margin if self.loss_mode == "corn_adaptive_tau_safe" else None,
                    "corn_soft_target_detached": self.corn_soft_detach_target if self.loss_mode == "corn_adaptive_tau_safe" else None,
                } if self.loss_mode in {"adaptive_ucl_cda_v3", "corn_adaptive_tau_safe"} else None
            ),
