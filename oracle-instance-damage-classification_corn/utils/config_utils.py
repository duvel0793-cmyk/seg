...
def _normalize_int_mapping(mapping: dict[Any, Any] | None) -> dict[int, int]:
    if mapping is None:
        return {}
    return {
...
def ensure_auxiliary_config_defaults(config: dict[str, Any]) -> dict[str, Any]:
    ...
    pixel_cfg = config.setdefault("pixel_auxiliary", {})
    pixel_cfg.setdefault("enabled", False)
    pixel_cfg.setdefault("train_only", True)
    pixel_cfg.setdefault("use_local_branch_only", True)
    pixel_cfg.setdefault("decoder_channels", 128)
    pixel_cfg.setdefault("use_pixel_bridge", True)

    building_cfg = pixel_cfg.setdefault("building_head", {})
    building_cfg.setdefault("enabled", True)
    building_cfg.setdefault("weight", 0.08)
    building_cfg.setdefault("loss", "bce_dice")
    building_cfg.setdefault("pos_weight", 2.0)
    building_cfg.setdefault("bce_weight", 0.5)
    building_cfg.setdefault("dice_weight", 0.5)

    damage_cfg = pixel_cfg.setdefault("damage_head", {})
    damage_cfg.setdefault("enabled", True)
    damage_cfg.setdefault("weight", 0.12)
    damage_cfg.setdefault("num_classes", 5)
    damage_cfg.setdefault("ignore_index", 0)
    damage_cfg.setdefault("class_weights", [0.2, 1.0, 1.35, 1.30, 1.0])
    damage_cfg.setdefault("loss", "ce_dice")
    damage_cfg.setdefault("ce_weight", 0.5)
    damage_cfg.setdefault("dice_weight", 0.5)
    damage_cfg["class_weights"] = _validate_class_weights(
        damage_cfg["class_weights"],
        num_classes=int(damage_cfg["num_classes"]),
        label="pixel_auxiliary.damage_head.class_weights",
    )

    supervision_cfg = pixel_cfg.setdefault("supervision", {})
    supervision_cfg.setdefault("prefer_true_pixel_target", True)
    supervision_cfg.setdefault("allow_rasterized_fallback", True)
    supervision_cfg.setdefault("xbd_target_subdir", "targets")
    supervision_cfg.setdefault("xbd_target_suffix", "_post_disaster_target.png")
    supervision_cfg.setdefault("xbd_target_value_to_damage_id", {0: 0, 1: 1, 2: 2, 3: 3, 4: 4})
    supervision_cfg.setdefault("bright_target_value_to_damage_id", {})
    supervision_cfg.setdefault("bright_nonzero_as_building", True)
    supervision_cfg["xbd_target_value_to_damage_id"] = _normalize_int_mapping(
        supervision_cfg.get("xbd_target_value_to_damage_id")
    )
    supervision_cfg["bright_target_value_to_damage_id"] = _normalize_int_mapping(
        supervision_cfg.get("bright_target_value_to_damage_id")
    )

    bridge_cfg = pixel_cfg.setdefault("bridge", {})
    bridge_cfg.setdefault("use_instance_mask_pooling", True)
    bridge_cfg.setdefault("use_boundary_stats", True)
    bridge_cfg.setdefault("detach_pixel_bridge", False)

    # Keep convenient mirrors in training/model blocks for lightweight logging and overrides.
    train_cfg.setdefault("lambda_building_aux", float(building_cfg["weight"]))
    train_cfg.setdefault("lambda_damage_aux", float(damage_cfg["weight"]))
    model_cfg.setdefault("pixel_auxiliary", {})
    model_cfg["pixel_auxiliary"] = copy.deepcopy(pixel_cfg)
    return config
