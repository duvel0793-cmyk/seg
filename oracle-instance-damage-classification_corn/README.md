...
The task definition is unchanged. This repository is still focused on **ordinal supervision / boundary ambiguity modeling** for `minor-damage` vs `major-damage`, but the default shared Siamese encoder has been upgraded from **ResNet18** to **ConvNeXt-Tiny**.

What stays the same:

- paired crop construction stays the same
- oracle GT instance crop stays the same
- shared Siamese encoder structure stays the same
- 4-channel branch input stays the same: `pre_rgb + oracle_mask`, `post_rgb + oracle_mask`
- `BidirectionalFusionBlock` stays the same
- `ChannelAttentionGate` stays the same
- `MaskedMultiScalePooling` stays the same
- `InstanceClassifierHead` / `OrdinalCORNHead` stay the same
- `OrdinalAmbiguityHead` stays the same
- CORN / adaptive tau / CDA losses and evaluation outputs stay the same

Only the shared feature backbone is swapped. By default the project now uses `convnextv2_tiny`, while `convnext_tiny` / `resnet18` remain available for compatibility.
...
- shared Siamese encoder with default `ConvNeXtV2-Tiny` backbone
...
- does **not** change the task, fusion path, pooling path, or evaluation protocol
...
- in safe mode, the ambiguity head also reads detached pooled features, so tau regularization does not push the shared `oracle_mcd` trunk through the tau branch
...
## Low-Intrusion Boundary Enhancements

The repository now supports three optional additions that keep the main oracle-instance task and train/eval flow unchanged:

- `model.aux_soft_label_head`:
  - adds a lightweight 4-class auxiliary distribution head on top of the pooled feature
  - trains with configurable ordinal soft labels such as `minor -> [0.15, 0.65, 0.20, 0.00]`
  - is training-only; evaluation still uses the original main head
- `model.ordinal_contrastive`:
  - adds a projection head on the pooled feature
  - applies a batch-internal order-aware pairwise margin regularizer:
    - same class: pull together
    - adjacent class: keep a small margin
    - far class: keep a larger margin
- `training.save_epoch_checkpoints`:
  - saves lightweight per-epoch checkpoints for later checkpoint soup / averaging
  - these checkpoints remain compatible with `evaluate.py`

Set the new weights to `0.0` or disable the corresponding modules to recover the previous behavior as closely as possible.
...
Default pretrained backbone loading uses:

- torchvision official ImageNet weights for `convnext_*`
- timm pretrained weights for `convnextv2_*` (install `timm`)

- first run with `pretrained: true` may auto-download pretrained backbone weights into `./weights/checkpoints/`
- if the server is offline, copy the cached weights into that directory ahead of time
- for fully offline training, set `pretrained: false`, but expect slower convergence and a lower upper bound

## Main Commands

```bash
python train.py --config configs/vscode_run.yaml
python evaluate.py --config configs/vscode_run.yaml
python scripts/smoke_test_convnext.py
```
...
python scripts/average_checkpoints.py \
  --config configs/corn_safe_aux_softlabel_contrastive.yaml \
  --checkpoints outputs/corn_safe_aux_softlabel_contrastive/oracle_mcd_corn/corn_adaptive_tau_safe/checkpoints/epoch_007.pth \
                outputs/corn_safe_aux_softlabel_contrastive/oracle_mcd_corn/corn_adaptive_tau_safe/checkpoints/epoch_008.pth \
                outputs/corn_safe_aux_softlabel_contrastive/oracle_mcd_corn/corn_adaptive_tau_safe/checkpoints/epoch_009.pth \
                outputs/corn_safe_aux_softlabel_contrastive/oracle_mcd_corn/corn_adaptive_tau_safe/checkpoints/epoch_010.pth \
  --output outputs/corn_safe_aux_softlabel_contrastive/oracle_mcd_corn/corn_adaptive_tau_safe/checkpoints/epoch_007_010_soup.pth

python evaluate.py \
  --config configs/corn_safe_aux_softlabel_contrastive.yaml \
  --checkpoint outputs/corn_safe_aux_softlabel_contrastive/oracle_mcd_corn/corn_adaptive_tau_safe/checkpoints/epoch_007_010_soup.pth
```
...
```bash
python train.py --config configs/vscode_run.yaml --batch_size 16
...
- image size: `224`
- batch size: `32`
- if ConvNeXt-Tiny triggers OOM on your GPU, try `--batch_size 16`
- epochs: `50`
- optimizer: `AdamW`
- lr: `3e-4`
- weight decay: `1e-4`
