...
def resolve_default_config_path() -> Path:
    preferred_candidates = [
        PROJECT_ROOT / "configs" / "image_recipe_from_screenshot_xbd.yaml",
        PROJECT_ROOT / "configs" / "method_upgrade_convnextv2_xbd.yaml",
        PROJECT_ROOT / "configs" / "vscode_run.yaml",
    ]
...
def parse_args() -> argparse.Namespace:
    ...
    add_optional_bool_argument("--aux_soft_label_enabled", default=None)
    parser.add_argument("--aux_soft_label_weight", type=float, default=None)
    parser.add_argument("--aux_soft_label_hidden_dim", type=int, default=None)
    ...
    parser.add_argument("--contrastive_distance", type=str, choices=["cosine", "euclidean"], default=None)
    parser.add_argument("--contrastive_margin_adjacent", type=float, default=None)
    parser.add_argument("--contrastive_margin_far", type=float, default=None)
    ...
    add_optional_bool_argument("--save_epoch_checkpoints", default=None)
...
def ensure_training_defaults(config: dict[str, Any], default_loss_mode: str = "corn_adaptive_tau_safe") -> dict[str, Any]:
    ...
    default_is_v3 = default_loss_mode_name in {"adaptive_ucl_cda_v3", "corn_adaptive_tau_safe", "corn_ordinal_multitask_v1"}
...
def load_and_override_config(args: argparse.Namespace) -> dict[str, Any]:
    ...
    aux_head_cfg = model_cfg.setdefault("aux_soft_label_head", {})
    contrastive_cfg = model_cfg.setdefault("ordinal_contrastive", {})
    if args.aux_soft_label_enabled is not None:
        aux_head_cfg["enabled"] = args.aux_soft_label_enabled
    if args.aux_soft_label_weight is not None:
        aux_head_cfg["weight"] = args.aux_soft_label_weight
    if args.aux_soft_label_hidden_dim is not None:
        aux_head_cfg["hidden_dim"] = args.aux_soft_label_hidden_dim
    ...
    for arg_name in [
        ...
        "contrastive_dropout",
        "contrastive_margin_adjacent",
        "contrastive_margin_far",
        ...
        if value is not None:
            train_cfg[arg_name] = value
    if args.contrastive_distance is not None:
        contrastive_cfg["distance"] = args.contrastive_distance
    if args.enable_ordinal_contrastive is not None:
        contrastive_cfg["enabled"] = args.enable_ordinal_contrastive
    if args.lambda_contrastive is not None:
        contrastive_cfg["weight"] = args.lambda_contrastive
    if args.contrastive_proj_dim is not None:
        contrastive_cfg["proj_dim"] = args.contrastive_proj_dim
    if args.contrastive_hidden_features is not None:
        contrastive_cfg["hidden_dim"] = args.contrastive_hidden_features
    if args.contrastive_dropout is not None:
        contrastive_cfg["dropout"] = args.contrastive_dropout
    if args.contrastive_margin_adjacent is not None:
        contrastive_cfg["margin_adjacent"] = args.contrastive_margin_adjacent
    if args.contrastive_margin_far is not None:
        contrastive_cfg["margin_far"] = args.contrastive_margin_far
    if args.contrastive_far_weight is not None:
        contrastive_cfg["far_weight"] = args.contrastive_far_weight
...
def make_dataloaders(
    ...
    *,
    ...
    train_dataset = build_instance_dataset(config, split_name="train", is_train=True)
    val_dataset = build_instance_dataset(
        config,
...
def unpack_model_outputs(model_output: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "local_building_logits": model_output.get("local_building_logits"),
        "local_damage_logits": model_output.get("local_damage_logits"),
        "pixel_bridge_feature": model_output.get("pixel_bridge_feature"),
...
def build_model_forward_kwargs(
    ...
    *,
    ...
    def _tensor(key: str) -> torch.Tensor | None:
        value = batch.get(f"{view_prefix}{key}")
    ...
    if kwargs["pre_image"] is None or kwargs["post_image"] is None or kwargs["instance_mask"] is None:
        raise KeyError(f"Missing required model inputs for prefix='{view_prefix}'.")
...
def binary_dice_loss_from_logits(
    ...
    safe_targets = targets.float()
    intersection = (probabilities * safe_targets).sum(dim=(1, 2, 3))
    denominator = probabilities.sum(dim=(1, 2, 3)) + safe_targets.sum(dim=(1, 2, 3))
    dice = (2.0 * intersection + eps) / denominator.clamp_min(eps)
...
def multiclass_dice_loss_from_logits(
    ...
    *,
    ...
    valid_mask = (safe_targets != int(ignore_index)).float().unsqueeze(1)
    losses: list[torch.Tensor] = []
    for class_index in range(probabilities.size(1)):
        if class_index == int(ignore_index):
            continue
        class_probability = probabilities[:, class_index : class_index + 1]
        class_target = (safe_targets == int(class_index)).float().unsqueeze(1)
        intersection = (class_probability * class_target * valid_mask).sum(dim=(1, 2, 3))
        denominator = (
            (class_probability * valid_mask).sum(dim=(1, 2, 3))
            + (class_target * valid_mask).sum(dim=(1, 2, 3))
        )
        class_dice = (2.0 * intersection + eps) / denominator.clamp_min(eps)
        losses.append(1.0 - class_dice.mean())
    if not losses:
...
def build_contrastive_loss_module(config: dict[str, Any], device: torch.device) -> OrderAwareContrastiveLoss | None:
    contrastive_cfg = config["model"].get("ordinal_contrastive", {})
    rank_cfg = config.get("ordinal_multitask", {}).get("rank_contrastive", {})
    enabled = bool(rank_cfg.get("enabled", contrastive_cfg.get("enabled", False)))
    if not enabled:
        return None
    pair_weights = rank_cfg.get("pair_weights", {})
    return OrderAwareContrastiveLoss(
        distance=str(rank_cfg.get("distance", contrastive_cfg.get("distance", "cosine"))),
        margin_adjacent=float(rank_cfg.get("margin_gap1", contrastive_cfg.get("margin_adjacent", 0.40))),
        margin_far=float(rank_cfg.get("margin_gap2", contrastive_cfg.get("margin_far", 0.80))),
        far_weight=float(pair_weights.get("gap2", contrastive_cfg.get("far_weight", 1.0))),
        margin_gap1=float(rank_cfg.get("margin_gap1", contrastive_cfg.get("margin_gap1", 0.40))),
        margin_gap2=float(rank_cfg.get("margin_gap2", contrastive_cfg.get("margin_gap2", 0.80))),
        margin_gap3=float(rank_cfg.get("margin_gap3", contrastive_cfg.get("margin_gap3", 1.10))),
        pair_weight_same=float(pair_weights.get("same", contrastive_cfg.get("pair_weight_same", 1.0))),
        pair_weight_gap1=float(pair_weights.get("gap1", contrastive_cfg.get("pair_weight_gap1", 1.2))),
        pair_weight_gap2=float(pair_weights.get("gap2", contrastive_cfg.get("pair_weight_gap2", 1.0))),
        pair_weight_gap3=float(pair_weights.get("gap3", contrastive_cfg.get("pair_weight_gap3", 1.1))),
        minor_major_pair_boost=float(
            rank_cfg.get("boost_minor_major_pairs", contrastive_cfg.get("minor_major_pair_boost", 1.1))
        ),
    ).to(device)
...
def resolve_tau_phase(config: dict[str, Any], epoch_index: int) -> dict[str, Any]:
    loss_mode = str(config["training"]["loss_mode"])
    if loss_mode not in {"adaptive_ucl_cda_v2", "adaptive_ucl_cda_v3", "corn_adaptive_tau_safe", "corn_ordinal_multitask_v1"}:
        return {
            "phase_name": "disabled",
            "use_fixed_tau": False,
    ...
    freeze_epochs = int(config["training"].get("tau_freeze_epochs", 1 if loss_mode in {"adaptive_ucl_cda_v3", "corn_adaptive_tau_safe", "corn_ordinal_multitask_v1"} else 3))
    warmup_value = float(config["training"].get("tau_warmup_value", 0.22 if loss_mode in {"adaptive_ucl_cda_v3", "corn_adaptive_tau_safe", "corn_ordinal_multitask_v1"} else 0.25))
...
def run_epoch(
    ...
    contrastive_loss_module: OrderAwareContrastiveLoss | None = None,
    ...
    pixel_auxiliary_cfg: dict[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    loss_terms = {
        ...
        "loss_severity_reg": 0.0,
        "loss_consistency_dist": 0.0,
        "loss_consistency_severity": 0.0,
        "loss_view_consistency": 0.0,
        "loss_view_severity_consistency": 0.0,
        "loss_building_aux": 0.0,
        "loss_damage_aux": 0.0,
    }
    aux_head_correct = 0.0
    aux_head_total = 0.0
    distribution_head_correct = 0.0
    distribution_head_total = 0.0
    contrastive_distance_sums = {
        "mean_same_distance": 0.0,
        "mean_gap1_distance": 0.0,
        "mean_gap2_distance": 0.0,
        "mean_gap3_distance": 0.0,
        "mean_adjacent_distance": 0.0,
        "mean_far_distance": 0.0,
    }
    contrastive_distance_counts = {
        "gap0_pair_count": 0.0,
        "gap1_pair_count": 0.0,
        "gap2_pair_count": 0.0,
        "gap3_pair_count": 0.0,
    ...
    ambiguity_input_detached = False
    distribution_expected_severity_values: list[float] = []
    pred_severity_values: list[float] = []
    ...
    pixel_aux_cfg = pixel_auxiliary_cfg or {}
    pixel_enabled = bool(pixel_aux_cfg.get("enabled", False))
    pixel_building_cfg = pixel_aux_cfg.get("building_head", {})
    pixel_damage_cfg = pixel_aux_cfg.get("damage_head", {})
    pixel_building_enabled = bool(pixel_enabled and pixel_building_cfg.get("enabled", True))
    pixel_damage_enabled = bool(pixel_enabled and pixel_damage_cfg.get("enabled", True))
    lambda_building_aux = float(pixel_building_cfg.get("weight", 0.08))
    lambda_damage_aux = float(pixel_damage_cfg.get("weight", 0.12))
    building_pos_weight = float(pixel_building_cfg.get("pos_weight", 2.0))
    building_bce_weight = float(pixel_building_cfg.get("bce_weight", 0.5))
    building_dice_weight = float(pixel_building_cfg.get("dice_weight", 0.5))
    damage_ignore_index = int(pixel_damage_cfg.get("ignore_index", 0))
    damage_ce_weight = float(pixel_damage_cfg.get("ce_weight", 0.5))
    damage_dice_weight = float(pixel_damage_cfg.get("dice_weight", 0.5))
    damage_num_classes = int(pixel_damage_cfg.get("num_classes", 5))
    damage_class_weights_cfg = pixel_damage_cfg.get("class_weights", [0.2, 1.0, 1.35, 1.30, 1.0])
    if len(damage_class_weights_cfg) != damage_num_classes:
        damage_class_weights_cfg = [1.0 for _ in range(damage_num_classes)]
    damage_class_weights = torch.tensor(damage_class_weights_cfg, dtype=torch.float32, device=device)
    pixel_supervised_samples = 0
    pixel_total_samples = 0
    building_intersection_sum = 0.0
    building_union_sum = 0.0
    damage_intersection_sums = [0.0 for _ in range(damage_num_classes)]
    damage_union_sums = [0.0 for _ in range(damage_num_classes)]
    contrastive_active = bool(
        contrastive_loss_module is not None
    ...
    for batch in progress:
        ...
        batch_source_names = [str(source) for source in batch.get("source_name", [])]
        ...
        with context:
            ...
            loss_outputs = criterion(
                ...
                distribution_logits=model_outputs.get("distribution_logits"),
                severity_score=model_outputs.get("severity_score"),
                sample_tau=tau_used,
            ...
            loss_building_aux = logits.new_tensor(0.0)
            loss_damage_aux = logits.new_tensor(0.0)
            batch_pixel_supervision_ratio = logits.new_tensor(0.0)
            batch_pixel_supervised_count = 0
            if pixel_enabled:
                has_pixel_supervision = batch.get("has_pixel_supervision")
                if has_pixel_supervision is not None:
                    pixel_supervision_mask = torch.as_tensor(
                        [bool(flag) for flag in has_pixel_supervision],
                        device=logits.device,
                        dtype=torch.bool,
                    )
                else:
                    pixel_supervision_mask = torch.zeros(labels.size(0), device=logits.device, dtype=torch.bool)

                supervised_count = int(pixel_supervision_mask.sum().item())
                batch_pixel_supervised_count = supervised_count
                pixel_total_samples += int(labels.size(0))
                pixel_supervised_samples += supervised_count
                batch_pixel_supervision_ratio = pixel_supervision_mask.float().mean()

                if supervised_count > 0:
                    if pixel_building_enabled and model_outputs.get("local_building_logits") is not None:
                        building_targets = batch.get("local_building_target")
                        if isinstance(building_targets, torch.Tensor):
                            selected_logits = model_outputs["local_building_logits"][pixel_supervision_mask]
                            selected_targets = building_targets.to(device, non_blocking=True)[pixel_supervision_mask].float()
                            bce_loss = F.binary_cross_entropy_with_logits(
                                selected_logits,
                                selected_targets,
                                pos_weight=selected_logits.new_tensor(building_pos_weight),
                            )
                            dice_loss = binary_dice_loss_from_logits(selected_logits, selected_targets)
                            loss_building_aux = (building_bce_weight * bce_loss) + (building_dice_weight * dice_loss)

                            building_prediction = (torch.sigmoid(selected_logits).detach() > 0.5)
                            building_target_mask = selected_targets.detach() > 0.5
                            batch_building_intersection = (building_prediction & building_target_mask).sum().item()
                            batch_building_union = (building_prediction | building_target_mask).sum().item()
                            building_intersection_sum += float(batch_building_intersection)
                            building_union_sum += float(batch_building_union)

                    if pixel_damage_enabled and model_outputs.get("local_damage_logits") is not None:
                        damage_targets = batch.get("local_damage_target")
                        if isinstance(damage_targets, torch.Tensor):
                            selected_logits = model_outputs["local_damage_logits"][pixel_supervision_mask]
                            selected_targets = damage_targets.to(device, non_blocking=True)[pixel_supervision_mask].long()
                            resolved_damage_classes = int(selected_logits.size(1))
                            resolved_damage_weights = damage_class_weights
                            if int(resolved_damage_weights.numel()) != resolved_damage_classes:
                                resolved_damage_weights = torch.ones(
                                    resolved_damage_classes,
                                    dtype=torch.float32,
                            ...
                            ce_loss = F.cross_entropy(
                                selected_logits,
                                selected_targets,
                                weight=resolved_damage_weights,
                                ignore_index=damage_ignore_index,
                            )
                            dice_loss = multiclass_dice_loss_from_logits(
                                selected_logits,
                                selected_targets,
                                ignore_index=damage_ignore_index,
                            )
                            loss_damage_aux = (damage_ce_weight * ce_loss) + (damage_dice_weight * dice_loss)

                            damage_prediction = selected_logits.detach().argmax(dim=1)
                            for class_index in range(resolved_damage_classes):
                                if class_index == damage_ignore_index:
                                    continue
                                pred_mask = damage_prediction == int(class_index)
                                target_mask = selected_targets == int(class_index)
                                if class_index < len(damage_intersection_sums):
                                    damage_intersection_sums[class_index] += float((pred_mask & target_mask).sum().item())
                                    damage_union_sums[class_index] += float((pred_mask | target_mask).sum().item())

                loss = loss + (lambda_building_aux * loss_building_aux) + (lambda_damage_aux * loss_damage_aux)
                loss_backward = (
                    loss_backward
                    + (lambda_building_aux * loss_building_aux)
                    + (lambda_damage_aux * loss_damage_aux)
                )
            ...
            if dual_view_active and "view2_pre_image" in batch:
                ...
                view2_model_outputs = unpack_model_outputs(model(**build_model_forward_kwargs(batch, device, view_prefix="view2_")))
                view2_logits = (
            ...
            if contrastive_active and contrastive_loss_module is not None and model_outputs.get("contrastive_embedding") is not None:
                ...
                contrastive_outputs = contrastive_loss_module(merged_embeddings, merged_labels)
                loss_contrastive = contrastive_outputs["loss"]
                loss = loss + (float(lambda_contrastive) * loss_contrastive)
                if is_train:
                    loss_backward = loss_backward + (float(lambda_contrastive) * loss_contrastive)
                loss_outputs.update(contrastive_outputs)
            else:
            ...
            loss_outputs["loss_building_aux"] = loss_building_aux
            loss_outputs["loss_damage_aux"] = loss_damage_aux
            loss_outputs["pixel_supervision_ratio"] = batch_pixel_supervision_ratio
        ...
        for name, value in (
            ...
            ("distribution_logits", model_outputs.get("distribution_logits")),
            ("severity_score", model_outputs.get("severity_score")),
            ("local_building_logits", model_outputs.get("local_building_logits")),
            ("local_damage_logits", model_outputs.get("local_damage_logits")),
            ("tau_used", tau_used),
            ...
            ("loss_aux_soft", loss_outputs.get("loss_aux_soft")),
            ("loss_distribution", loss_outputs.get("loss_distribution")),
            ("loss_severity_reg", loss_outputs.get("loss_severity_reg")),
            ("loss_consistency_dist", loss_outputs.get("loss_consistency_dist")),
            ("loss_consistency_severity", loss_outputs.get("loss_consistency_severity")),
            ("loss_view_consistency", loss_outputs.get("loss_view_consistency")),
            ("loss_building_aux", loss_outputs.get("loss_building_aux")),
            ("loss_damage_aux", loss_outputs.get("loss_damage_aux")),
        ):
            if value is None:
                continue
        ...
        if is_train:
            ...
            if (
                criterion.loss_mode in {"corn_adaptive_tau_safe", "corn_ordinal_multitask_v1"}
        ...
        for source_name in batch_source_names:
            source_sample_counts[source_name] = source_sample_counts.get(source_name, 0) + 1
        ...
        if model_outputs.get("aux_logits") is not None:
            aux_predictions = model_outputs["aux_logits"].detach().argmax(dim=1)
            aux_head_correct += float((aux_predictions == labels).sum().item())
            aux_head_total += float(labels.numel())
        if loss_outputs.get("distribution_probabilities") is not None:
            distribution_predictions = loss_outputs["distribution_probabilities"].detach().argmax(dim=1)
            distribution_head_correct += float((distribution_predictions == labels).sum().item())
            distribution_head_total += float(labels.numel())
        if loss_outputs.get("distribution_expected_severity") is not None:
            distribution_expected_severity_values.extend(
                float(value) for value in loss_outputs["distribution_expected_severity"].detach().float().cpu().tolist()
            )
        if model_outputs.get("severity_score") is not None:
            pred_severity_values.extend(
                float(value) for value in model_outputs["severity_score"].detach().float().cpu().tolist()
            )
        for mean_key, count_key in [
            ("mean_same_distance", "same_pair_count"),
            ("mean_gap1_distance", "gap1_pair_count"),
            ("mean_gap2_distance", "gap2_pair_count"),
            ("mean_gap3_distance", "gap3_pair_count"),
            ("mean_adjacent_distance", "adjacent_pair_count"),
            ("mean_far_distance", "far_pair_count"),
        ]:
            pair_count = float(loss_outputs.get(count_key, logits.new_tensor(0.0)).detach().item())
            if pair_count <= 0.0:
                continue
            contrastive_distance_sums[mean_key] += (
                float(loss_outputs.get(mean_key, logits.new_tensor(0.0)).detach().item()) * pair_count
            )
            contrastive_distance_counts[count_key] += pair_count
        contrastive_distance_counts["gap0_pair_count"] = contrastive_distance_counts["same_pair_count"]
        ...
        if criterion.loss_mode in {"corn_adaptive_tau_safe", "corn_ordinal_multitask_v1"}:
        ...
        if loss_outputs["loss_distribution"].detach().item() > 0:
            postfix["dist"] = f"{loss_outputs['loss_distribution'].detach().item():.4f}"
        if loss_outputs["loss_severity_reg"].detach().item() > 0:
            postfix["reg"] = f"{loss_outputs['loss_severity_reg'].detach().item():.4f}"
        if loss_outputs["loss_consistency_dist"].detach().item() > 0:
            postfix["cons_d"] = f"{loss_outputs['loss_consistency_dist'].detach().item():.4f}"
        if loss_outputs["loss_consistency_severity"].detach().item() > 0:
            postfix["cons_s"] = f"{loss_outputs['loss_consistency_severity'].detach().item():.4f}"
        if loss_outputs["loss_building_aux"].detach().item() > 0:
            postfix["pix_b"] = f"{loss_outputs['loss_building_aux'].detach().item():.4f}"
        if loss_outputs["loss_damage_aux"].detach().item() > 0:
            postfix["pix_d"] = f"{loss_outputs['loss_damage_aux'].detach().item():.4f}"
        if pixel_enabled:
            postfix["pix_r"] = f"{float(loss_outputs.get('pixel_supervision_ratio', 0.0).detach().item()):.2f}"
            postfix["pix_n"] = str(int(batch_pixel_supervised_count))
        if tau_used is not None:
        ...
        if loss_outputs["loss_aux_soft"].detach().item() > 0:
            postfix["aux"] = f"{loss_outputs['loss_aux_soft'].detach().item():.4f}"
        ...
        if batch_source_names:
            batch_source_summary: dict[str, int] = {}
            for source_name in batch_source_names:
                batch_source_summary[source_name] = batch_source_summary.get(source_name, 0) + 1
            postfix["src"] = ",".join(f"{name}:{count}" for name, count in sorted(batch_source_summary.items()))
    ...
    metrics["corr_raw_tau_difficulty"] = (
        ...
        if getattr(criterion, "loss_mode", "") in {"adaptive_ucl_cda_v3", "corn_adaptive_tau_safe", "corn_ordinal_multitask_v1"}
        else None
    )
    metrics["raw_tau_edge_ratio"] = (
        ...
        if (
            getattr(criterion, "loss_mode", "") in {"adaptive_ucl_cda_v3", "corn_adaptive_tau_safe", "corn_ordinal_multitask_v1"}
    ...
    metrics["aux_head_acc"] = (aux_head_correct / max(aux_head_total, 1.0)) if aux_head_total > 0 else None
    metrics["distribution_head_acc"] = (
        distribution_head_correct / max(distribution_head_total, 1.0)
        if distribution_head_total > 0
        else None
    )
    metrics["distribution_expected_severity_stats"] = summarize_mean_std_min_max(distribution_expected_severity_values)
    metrics["pred_severity_stats"] = summarize_mean_std_min_max(pred_severity_values)
    metrics["aux_soft_label_weight"] = float(getattr(criterion, "aux_soft_label_weight", 0.0))
    metrics["contrastive_weight"] = float(lambda_contrastive) if contrastive_active else float(lambda_contrastive)
    metrics["mean_same_distance"] = (
        contrastive_distance_sums["mean_same_distance"] / max(contrastive_distance_counts["same_pair_count"], 1.0)
        if contrastive_distance_counts["same_pair_count"] > 0
        else None
    )
    metrics["mean_adjacent_distance"] = (
        contrastive_distance_sums["mean_adjacent_distance"] / max(contrastive_distance_counts["adjacent_pair_count"], 1.0)
        if contrastive_distance_counts["adjacent_pair_count"] > 0
        else None
    )
    metrics["mean_gap1_distance"] = (
        contrastive_distance_sums["mean_gap1_distance"] / max(contrastive_distance_counts["gap1_pair_count"], 1.0)
        if contrastive_distance_counts["gap1_pair_count"] > 0
        else None
    )
    metrics["mean_gap2_distance"] = (
        contrastive_distance_sums["mean_gap2_distance"] / max(contrastive_distance_counts["gap2_pair_count"], 1.0)
        if contrastive_distance_counts["gap2_pair_count"] > 0
        else None
    )
    metrics["mean_gap3_distance"] = (
        contrastive_distance_sums["mean_gap3_distance"] / max(contrastive_distance_counts["gap3_pair_count"], 1.0)
        if contrastive_distance_counts["gap3_pair_count"] > 0
        else None
    )
    metrics["mean_far_distance"] = (
        contrastive_distance_sums["mean_far_distance"] / max(contrastive_distance_counts["far_pair_count"], 1.0)
        if contrastive_distance_counts["far_pair_count"] > 0
        else None
    )
    metrics["contrastive_pair_counts"] = {
        key: float(value)
        for key, value in contrastive_distance_counts.items()
    }
    metrics["pair_count_by_gap"] = {
        "gap0": float(contrastive_distance_counts.get("gap0_pair_count", 0.0)),
        "gap1": float(contrastive_distance_counts.get("gap1_pair_count", 0.0)),
        "gap2": float(contrastive_distance_counts.get("gap2_pair_count", 0.0)),
        "gap3": float(contrastive_distance_counts.get("gap3_pair_count", 0.0)),
    }
    metrics["corn_soft_enabled"] = bool(corn_soft_enabled and criterion.loss_mode in {"corn_adaptive_tau_safe", "corn_ordinal_multitask_v1"})
    metrics["corn_mode"] = criterion.loss_mode if criterion.loss_mode in {"corn", "corn_adaptive_tau_safe", "corn_ordinal_multitask_v1"} else None
    ...
    metrics["source_counts"] = source_sample_counts
    metrics["dual_view_enabled"] = bool(dual_view_active)
    metrics["contrastive_enabled"] = bool(contrastive_active)
    metrics["pixel_supervision_ratio"] = (
        float(pixel_supervised_samples) / max(float(pixel_total_samples), 1.0)
        if pixel_enabled and pixel_total_samples > 0
        else 0.0
    )
    metrics["building_iou_aux"] = (
        float(building_intersection_sum / max(building_union_sum, 1.0))
        if pixel_enabled and building_union_sum > 0.0
        else None
    )
    damage_class_ious = [
        damage_intersection_sums[class_index] / max(damage_union_sums[class_index], 1.0)
        for class_index in range(damage_num_classes)
        if class_index != damage_ignore_index and damage_union_sums[class_index] > 0.0
    ]
    metrics["damage_mIoU_aux"] = (
        float(sum(damage_class_ious) / len(damage_class_ious))
        if pixel_enabled and damage_class_ious
        else None
    )
...
def make_epoch_checkpoint_snapshot(
    checkpoint_state: dict[str, Any],
    *,
    train_metrics: dict[str, Any],
    val_metrics: dict[str, Any],
...
def maybe_save_epoch_checkpoint(
    ...
    *,
    ...
    if not bool(config["training"].get("save_epoch_checkpoints", False)):
        return
    every_n = max(int(config["training"].get("save_every_n_epochs", 1)), 1)
    epoch = int(checkpoint_state["epoch"])
    if epoch % every_n != 0:
        return
    save_checkpoint(
        checkpoints_dir / f"epoch_{epoch:03d}.pth",
        make_epoch_checkpoint_snapshot(
            checkpoint_state,
...
def configure_stage2_trainability(
    ...
    if hasattr(model, "get_auxiliary_head_parameters"):
        set_requires_grad(list(model.get_auxiliary_head_parameters()), False)
...
def prepare_stage2_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    stage2_config["training"]["lambda_distribution"] = 0.0
    stage2_config["training"]["lambda_severity_reg"] = 0.0
    stage2_config["training"]["lambda_consistency_dist"] = 0.0
    stage2_config["training"]["lambda_consistency_severity"] = 0.0
    stage2_config["training"]["lambda_view_consistency"] = 0.0
    stage2_config["training"]["lambda_view_severity_consistency"] = 0.0
    stage2_config.setdefault("model", {}).setdefault("ordinal_contrastive", {})["enabled"] = False
    stage2_config.setdefault("model", {}).setdefault("aux_soft_label_head", {})["enabled"] = False
    stage2_multitask_cfg = stage2_config.setdefault("ordinal_multitask", {})
    stage2_multitask_cfg.setdefault("distribution_head", {})["enabled"] = False
    stage2_multitask_cfg.setdefault("severity_regression", {})["enabled"] = False
    stage2_multitask_cfg.setdefault("rank_contrastive", {})["enabled"] = False
    stage2_multitask_cfg.setdefault("consistency", {})["enabled"] = False
    stage2_pixel_cfg = stage2_config.setdefault("pixel_auxiliary", {})
    stage2_pixel_cfg["enabled"] = False
    stage2_pixel_cfg.setdefault("building_head", {})["weight"] = 0.0
    stage2_pixel_cfg.setdefault("damage_head", {})["weight"] = 0.0
    return ensure_auxiliary_config_defaults(stage2_config)
...
def run_stage2_head_rebalance(
    *,
    ...
    train_dataset = build_instance_dataset(stage2_config, split_name="train", is_train=True)
    val_dataset = build_instance_dataset(
        stage2_config,
        split_name=str(stage2_config["evaluation"]["split_name"]),
        is_train=False,
    )
    ...
    if model_load_status["missing_keys"] or model_load_status["unexpected_keys"]:
        print(
            "[Stage2] model load compatibility "
    ...
    for epoch in range(total_epochs):
        ...
        corn_soft_enabled = bool(
            str(stage2_config["training"]["loss_mode"]) in {"corn_adaptive_tau_safe", "corn_ordinal_multitask_v1"}
        ...
        train_metrics = run_epoch(
            ...
            pixel_auxiliary_cfg=stage2_config.get("pixel_auxiliary", {}),
        )
...
def _run_training(
    ...
    default_ambiguity_lr_scale = 0.30 if loss_mode in {"adaptive_ucl_cda_v3", "corn_adaptive_tau_safe", "corn_ordinal_multitask_v1"} else 0.5
    ...
    print(f"Train source: {config['data'].get('train_source', 'xbd_only')}")
    ...
    print(
        "Input mode: "
        f"local_size={int(config.get('input_mode', {}).get('local_size', config['data']['image_size']))} "
        f"context_size={int(config.get('input_mode', {}).get('context_size', config['data']['image_size']))} "
        f"local_margin_ratio={float(config.get('input_mode', {}).get('local_margin_ratio', 0.15)):.3f} "
        f"context_scale={float(config.get('input_mode', {}).get('context_scale', 2.5)):.3f} "
        f"min_crop_size={int(config.get('input_mode', {}).get('min_crop_size', 32))}"
    )
    print(
        "Geometry prior: "
        f"encoder_in_channels={resolved_encoder_in_channels} "
        f"boundary_width_px={int(config.get('geometry_prior', {}).get('boundary_width_px', 3))} "
        f"dilation_radius={int(config.get('geometry_prior', {}).get('dilation_radius', 3))} "
        f"erosion_radius={int(config.get('geometry_prior', {}).get('erosion_radius', 1))}"
    )
    ...
    print(f"aux_soft_label_enabled={bool(config['model'].get('aux_soft_label_head', {}).get('enabled', False))}")
    print(f"aux_soft_label_weight={float(config['model'].get('aux_soft_label_head', {}).get('weight', 0.0)):.4f}")
    print(f"contrastive_enabled={bool(config['model'].get('ordinal_contrastive', {}).get('enabled', False))}")
    print(f"contrastive_weight={float(config['model'].get('ordinal_contrastive', {}).get('weight', 0.0)):.4f}")
    print(f"contrastive_loss_warmup_epochs={int(config['training'].get('contrastive_loss_warmup_epochs', 3))}")
    print(f"distribution_head_enabled={bool(config.get('ordinal_multitask', {}).get('distribution_head', {}).get('enabled', False))}")
    print(f"severity_regression_enabled={bool(config.get('ordinal_multitask', {}).get('severity_regression', {}).get('enabled', False))}")
    print(f"consistency_enabled={bool(config.get('ordinal_multitask', {}).get('consistency', {}).get('enabled', False))}")
    print(f"lambda_distribution={float(config['training'].get('lambda_distribution', 0.0)):.4f}")
    print(f"lambda_severity_reg={float(config['training'].get('lambda_severity_reg', 0.0)):.4f}")
    print(f"lambda_consistency_dist={float(config['training'].get('lambda_consistency_dist', 0.0)):.4f}")
    print(f"lambda_consistency_severity={float(config['training'].get('lambda_consistency_severity', 0.0)):.4f}")
    print(f"pixel_auxiliary_enabled={bool(config.get('pixel_auxiliary', {}).get('enabled', False))}")
    print(f"pixel_aux_train_only={bool(config.get('pixel_auxiliary', {}).get('train_only', True))}")
    print(
        f"pixel_bridge_enabled={bool(config.get('pixel_auxiliary', {}).get('use_pixel_bridge', False))} "
        f"detach_pixel_bridge={bool(config.get('pixel_auxiliary', {}).get('bridge', {}).get('detach_pixel_bridge', False))}"
    )
    print(
        f"lambda_building_aux={float(config.get('pixel_auxiliary', {}).get('building_head', {}).get('weight', 0.0)):.4f}"
    )
    print(
        f"lambda_damage_aux={float(config.get('pixel_auxiliary', {}).get('damage_head', {}).get('weight', 0.0)):.4f}"
    )
    print(f"Early stop patience: {early_stop_patience}")
    print(f"Save top-k checkpoints: {save_topk}")
    print(f"Save epoch checkpoints: {bool(config['training'].get('save_epoch_checkpoints', False))}")
    print(f"Ambiguity input detached in safe mode: {loss_mode in {'corn_adaptive_tau_safe', 'corn_ordinal_multitask_v1'}}")
    ...
    print(f"Train source counts: {getattr(train_dataset, 'source_counts', {})}")
    print(f"Val source counts: {getattr(val_dataset, 'source_counts', {})}")
    ...
    for epoch in range(total_epochs):
        ...
        corn_soft_enabled = bool(loss_mode in {"corn_adaptive_tau_safe", "corn_ordinal_multitask_v1"} and (epoch + 1) >= corn_soft_start_epoch)
        ...
        train_metrics = run_epoch(
            ...
            pixel_auxiliary_cfg=config.get("pixel_auxiliary", {}),
        )
        val_metrics = run_epoch(
            model=model,
            loader=val_loader,
        ...
        print(
            ...
            f"aux_soft={train_metrics['loss_terms']['loss_aux_soft']:.4f} "
            f"dist={train_metrics['loss_terms']['loss_distribution']:.4f} "
            f"sev_reg={train_metrics['loss_terms']['loss_severity_reg']:.4f} "
            f"cons_dist={train_metrics['loss_terms']['loss_consistency_dist']:.4f} "
            f"cons_sev={train_metrics['loss_terms']['loss_consistency_severity']:.4f} "
            f"pix_b={train_metrics['loss_terms']['loss_building_aux']:.4f} "
            f"pix_d={train_metrics['loss_terms']['loss_damage_aux']:.4f} "
            ...
            f"aux_acc={(train_metrics['aux_head_acc'] if train_metrics['aux_head_acc'] is not None else 0.0):.4f} "
            f"dist_acc={(train_metrics['distribution_head_acc'] if train_metrics['distribution_head_acc'] is not None else 0.0):.4f} "
            f"acc={train_metrics['overall_accuracy']:.4f} "
            f"macro_f1={train_metrics['macro_f1']:.4f} "
            f"far_error={train_metrics['far_error_rate']:.4f} "
            f"pixel_ratio={float(train_metrics.get('pixel_supervision_ratio', 0.0)):.4f} "
            f"building_iou_aux={(train_metrics.get('building_iou_aux') if train_metrics.get('building_iou_aux') is not None else 0.0):.4f} "
            f"damage_mIoU_aux={(train_metrics.get('damage_mIoU_aux') if train_metrics.get('damage_mIoU_aux') is not None else 0.0):.4f} "
        ...
        print(
            ...
            f"aux_soft={val_metrics['loss_terms']['loss_aux_soft']:.4f} "
            f"dist={val_metrics['loss_terms']['loss_distribution']:.4f} "
            f"sev_reg={val_metrics['loss_terms']['loss_severity_reg']:.4f} "
            f"cons_dist={val_metrics['loss_terms']['loss_consistency_dist']:.4f} "
            f"cons_sev={val_metrics['loss_terms']['loss_consistency_severity']:.4f} "
            f"pix_b={val_metrics['loss_terms']['loss_building_aux']:.4f} "
            f"pix_d={val_metrics['loss_terms']['loss_damage_aux']:.4f} "
            ...
            f"aux_acc={(val_metrics['aux_head_acc'] if val_metrics['aux_head_acc'] is not None else 0.0):.4f} "
            f"dist_acc={(val_metrics['distribution_head_acc'] if val_metrics['distribution_head_acc'] is not None else 0.0):.4f} "
            f"acc={val_metrics['overall_accuracy']:.4f} "
            f"macro_f1={val_metrics['macro_f1']:.4f} "
            f"far_error={val_metrics['far_error_rate']:.4f} "
            f"pixel_ratio={float(val_metrics.get('pixel_supervision_ratio', 0.0)):.4f} "
            f"building_iou_aux={(val_metrics.get('building_iou_aux') if val_metrics.get('building_iou_aux') is not None else 0.0):.4f} "
            f"damage_mIoU_aux={(val_metrics.get('damage_mIoU_aux') if val_metrics.get('damage_mIoU_aux') is not None else 0.0):.4f} "
        ...
        print(f"Train distribution_expected_severity_stats={train_metrics.get('distribution_expected_severity_stats')}")
        print(f"Val   distribution_expected_severity_stats={val_metrics.get('distribution_expected_severity_stats')}")
        print(f"Train pred_severity_stats={train_metrics.get('pred_severity_stats')}")
        print(f"Val   pred_severity_stats={val_metrics.get('pred_severity_stats')}")
        print(
            "Train detach_flags "
        ...
        print(
            "Train contrastive_distances "
            f"same={train_metrics.get('mean_same_distance')} "
            f"gap1={train_metrics.get('mean_gap1_distance')} "
            f"gap2={train_metrics.get('mean_gap2_distance')} "
            f"gap3={train_metrics.get('mean_gap3_distance')} "
            f"adj={train_metrics.get('mean_adjacent_distance')} "
            f"far={train_metrics.get('mean_far_distance')}"
        )
        print(
            "Val   contrastive_distances "
            f"same={val_metrics.get('mean_same_distance')} "
            f"gap1={val_metrics.get('mean_gap1_distance')} "
            f"gap2={val_metrics.get('mean_gap2_distance')} "
            f"gap3={val_metrics.get('mean_gap3_distance')} "
            f"adj={val_metrics.get('mean_adjacent_distance')} "
            f"far={val_metrics.get('mean_far_distance')}"
        )
        print(
            "Train contrastive_pair_counts "
            f"gap0={train_metrics.get('pair_count_by_gap', {}).get('gap0', 0.0):.1f} "
            f"gap1={train_metrics.get('pair_count_by_gap', {}).get('gap1', 0.0):.1f} "
            f"gap2={train_metrics.get('pair_count_by_gap', {}).get('gap2', 0.0):.1f} "
            f"gap3={train_metrics.get('pair_count_by_gap', {}).get('gap3', 0.0):.1f} "
            f"same={train_metrics.get('contrastive_pair_counts', {}).get('same_pair_count', 0.0):.1f} "
            f"adj={train_metrics.get('contrastive_pair_counts', {}).get('adjacent_pair_count', 0.0):.1f} "
            f"far={train_metrics.get('contrastive_pair_counts', {}).get('far_pair_count', 0.0):.1f}"
        )
        print(
            "Val   contrastive_pair_counts "
            f"gap0={val_metrics.get('pair_count_by_gap', {}).get('gap0', 0.0):.1f} "
            f"gap1={val_metrics.get('pair_count_by_gap', {}).get('gap1', 0.0):.1f} "
            f"gap2={val_metrics.get('pair_count_by_gap', {}).get('gap2', 0.0):.1f} "
            f"gap3={val_metrics.get('pair_count_by_gap', {}).get('gap3', 0.0):.1f} "
            f"same={val_metrics.get('contrastive_pair_counts', {}).get('same_pair_count', 0.0):.1f} "
            f"adj={val_metrics.get('contrastive_pair_counts', {}).get('adjacent_pair_count', 0.0):.1f} "
            f"far={val_metrics.get('contrastive_pair_counts', {}).get('far_pair_count', 0.0):.1f}"
        )
