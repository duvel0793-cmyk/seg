...
def run_inference(
    ...
    pixel_cfg = pixel_auxiliary_cfg or {}
    pixel_eval_enabled = bool(pixel_cfg.get("enabled", False) and not pixel_cfg.get("train_only", True))
    pixel_damage_cfg = pixel_cfg.get("damage_head", {})
    ...
    building_pred_sum = 0.0
    building_target_sum = 0.0
    damage_correct = 0.0
    damage_total = 0.0
        ...
        for batch in tqdm(loader, leave=False, desc="eval"):
            ...
            if pixel_eval_enabled:
                ...
                if has_pixel_supervision is not None:
                    ...
                    pixel_supervision_mask = torch.zeros(labels.size(0), device=labels.device, dtype=torch.bool)
                ...
                if supervised_count > 0:
                    if model_outputs.get("local_building_logits") is not None and isinstance(batch.get("local_building_target"), torch.Tensor):
                        building_logits = model_outputs["local_building_logits"][pixel_supervision_mask]
                        building_targets = batch["local_building_target"].to(device, non_blocking=True)[pixel_supervision_mask].float()
                        building_pred = (torch.sigmoid(building_logits).detach() > 0.5)
                        building_target = building_targets.detach() > 0.5
                        building_intersection_sum += float((building_pred & building_target).sum().item())
                        building_union_sum += float((building_pred | building_target).sum().item())
                        building_pred_sum += float(building_pred.sum().item())
                        building_target_sum += float(building_target.sum().item())

                    if model_outputs.get("local_damage_logits") is not None and isinstance(batch.get("local_damage_target"), torch.Tensor):
                        damage_logits = model_outputs["local_damage_logits"][pixel_supervision_mask]
                        damage_targets = batch["local_damage_target"].to(device, non_blocking=True)[pixel_supervision_mask].long()
                        resolved_damage_classes = int(damage_logits.size(1))
                        damage_prediction = damage_logits.detach().argmax(dim=1)
                        valid_mask = damage_targets != int(damage_ignore_index)
                        if valid_mask.any():
                            damage_correct += float((damage_prediction[valid_mask] == damage_targets[valid_mask]).sum().item())
                            damage_total += float(valid_mask.sum().item())
                        for class_index in range(resolved_damage_classes):
                            ...
                            target_mask = damage_targets == int(class_index)
    ...
    return all_targets, all_predictions, prediction_records, {
        ...
        "pixel_supervision_ratio": (
            float(pixel_supervised_samples) / max(float(pixel_total_samples), 1.0)
            if pixel_eval_enabled and pixel_total_samples > 0
            else 0.0
        ),
        "building_iou_aux": (
            float(building_intersection_sum / max(building_union_sum, 1.0))
            if pixel_eval_enabled and building_union_sum > 0.0
            else None
        ),
        "building_dice_aux": (
            float((2.0 * building_intersection_sum) / max(building_pred_sum + building_target_sum, 1.0))
            if pixel_eval_enabled and (building_pred_sum + building_target_sum) > 0.0
            else None
        ),
        "damage_pixel_acc_aux": (
            float(damage_correct / max(damage_total, 1.0))
            if pixel_eval_enabled and damage_total > 0.0
            else None
        ),
        "damage_mIoU_aux": (
            float(sum(damage_class_ious) / len(damage_class_ious))
            if pixel_eval_enabled and damage_class_ious
            else None
        ),
    }, {
...
def main() -> None:
    ...
    metrics["pixel_supervision_ratio"] = inference_summary.get("pixel_supervision_ratio")
    metrics["building_iou_aux"] = inference_summary.get("building_iou_aux")
    metrics["building_dice_aux"] = inference_summary.get("building_dice_aux")
    metrics["damage_pixel_acc_aux"] = inference_summary.get("damage_pixel_acc_aux")
    metrics["damage_mIoU_aux"] = inference_summary.get("damage_mIoU_aux")
    ...
    write_json(
        eval_dir / "multitask_diagnostics.json",
        {
            ...
            "pixel_supervision_ratio": metrics.get("pixel_supervision_ratio"),
            "building_iou_aux": metrics.get("building_iou_aux"),
            "building_dice_aux": metrics.get("building_dice_aux"),
            "damage_pixel_acc_aux": metrics.get("damage_pixel_acc_aux"),
            "damage_mIoU_aux": metrics.get("damage_mIoU_aux"),
        },
    )
    ...
    if metrics.get("building_iou_aux") is not None:
        print(f"building_iou_aux={float(metrics['building_iou_aux']):.4f}")
    if metrics.get("building_dice_aux") is not None:
        print(f"building_dice_aux={float(metrics['building_dice_aux']):.4f}")
    if metrics.get("damage_pixel_acc_aux") is not None:
        print(f"damage_pixel_acc_aux={float(metrics['damage_pixel_acc_aux']):.4f}")
    if metrics.get("damage_mIoU_aux") is not None:
        print(f"damage_mIoU_aux={float(metrics['damage_mIoU_aux']):.4f}")
