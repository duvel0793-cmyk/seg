...
class BrightOpticalInstanceDamageDataset(Dataset):
    def __init__(
        ...
        self.bright_nonzero_as_building = bool(self.pixel_supervision_cfg.get("bright_nonzero_as_building", True))
        raw_bright_pixel_mapping = self.pixel_supervision_cfg.get("bright_target_value_to_damage_id", {})
        self.bright_target_value_to_damage_id = {
            int(raw_value): int(mapped_value)
            for raw_value, mapped_value in raw_bright_pixel_mapping.items()
        }
        ...
        self.damage_name_to_id = {
            "no-damage": 1,
            "minor-damage": 2,
            "major-damage": 3,
            "destroyed": 4,
        }
        if not self.bright_target_value_to_damage_id:
            self.bright_target_value_to_damage_id = {
                int(raw_value): int(self.damage_name_to_id[class_name])
                for raw_value, class_name in self.label_mapping_names.items()
            }
    ...
    def _build_samples(self) -> list[dict[str, Any]]:
        ...
        for tile_id in tqdm(split_ids, desc=f"Indexing bright-{self.split_name}", leave=False):
            ...
            if tile is None:
                continue
            try:
                with Image.open(tile["pre_image"]) as pre_handle:
                    pre_handle.verify()
                with Image.open(tile["post_image"]) as post_handle:
                    post_handle.verify()
    ...
    def _map_bright_damage_values(self, target_array: np.ndarray) -> np.ndarray:
        mapped = np.zeros_like(target_array, dtype=np.uint8)
        for raw_value, mapped_value in self.bright_target_value_to_damage_id.items():
    ...
    def _build_local_pixel_supervision(
        ...
        local_component_mask_image: Image.Image,
        ...
        if self.prefer_true_pixel_target:
            mapped_target = self._map_bright_damage_values(target_array)
            ...
            if self.bright_nonzero_as_building:
                building_array = (local_damage_target_array > 0).astype(np.uint8)
            else:
                building_array = (local_damage_target_array > 0).astype(np.uint8)
            return {
                "local_building_target_image": Image.fromarray(building_array, mode="L"),
                "local_damage_target_image": local_damage_target_image,
                "has_pixel_supervision": True,
                "pixel_source_name": "bright",
            }

        if self.allow_rasterized_fallback:
            local_component_mask_array = (np.asarray(local_component_mask_image, dtype=np.uint8) > 0).astype(np.uint8)
            weak_damage_array = local_component_mask_array * int(sample["label"] + 1)
            return {
                "local_building_target_image": Image.fromarray(local_component_mask_array, mode="L"),
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        local_component_mask_image = self._extract_component_mask(target_binary_image, local_crop_box, anchor_xy)
        local_pixel_targets = self._build_local_pixel_supervision(
            sample,
            local_crop_box,
            local_component_mask_image,
            target_array,
        )
