...
def oracle_instance_collate_fn(batch: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if "view2_pre_image" in batch[0]:
        collated["view2_pre_image"] = torch.stack([item["view2_pre_image"] for item in batch], dim=0)
        collated["view2_post_image"] = torch.stack([item["view2_post_image"] for item in batch], dim=0)
        collated["view2_instance_mask"] = torch.stack([item["view2_instance_mask"] for item in batch], dim=0)
    if "augmentation_stats" in batch[0]:
        collated["augmentation_stats"] = [item["augmentation_stats"] for item in batch]
    if "view2_augmentation_stats" in batch[0]:
        collated["view2_augmentation_stats"] = [item["view2_augmentation_stats"] for item in batch]
    return collated
...
class XBDOracleInstanceDamageDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        if self.dual_view_enabled and hasattr(self.transform, "make_dual_views"):
            view1, view2 = self.transform.make_dual_views(
                pre_crop,
                post_crop,
                mask,
        ...
        if view2_mask_tensor is not None and view2_mask_tensor.sum().item() <= 0:
        ...
        if view2_pre_tensor is not None and view2_post_tensor is not None and view2_mask_tensor is not None:
            sample_dict["view2_pre_image"] = view2_pre_tensor
            sample_dict["view2_post_image"] = view2_post_tensor
            sample_dict["view2_instance_mask"] = view2_mask_tensor
            sample_dict["view2_augmentation_stats"] = view2_augmentation_stats
        return sample_dict
