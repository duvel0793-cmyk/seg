...
class TrainPairTransform:
    ...
    def _resize(
    ...
    ) -> tuple[Image.Image, Image.Image, Image.Image, list[Image.Image] | None]:
        ...
        if extra_masks is None:
            return pre, post, mask, None
        resized_extra_masks = [
            TF.resize(extra_mask, size, interpolation=InterpolationMode.NEAREST)
            for extra_mask in extra_masks
        ]
        return pre, post, mask, resized_extra_masks
    ...
    def _safe_random_resized_crop(
        ...
        if original_mask_pixels <= 0:
            return self._resize(pre, post, mask, extra_masks=extra_masks)
        ...
        for _ in range(10):
            ...
            if retained >= self.min_mask_retention:
                ...
                if extra_masks is None:
                    return pre_out, post_out, mask_out, None
                extra_mask_outs = [
                    TF.resized_crop(
                        extra_mask,
                        top,
                ...
                return pre_out, post_out, mask_out, extra_mask_outs
    ...
    def prepare_augmented_pair(
        ...
        if random.random() < self.hflip_prob:
            ...
            if extra_masks is not None:
                extra_masks = [TF.hflip(extra_mask) for extra_mask in extra_masks]
        ...
        if random.random() < self.vflip_prob:
            ...
            mask = TF.vflip(mask)
            if extra_masks is not None:
                extra_masks = [TF.vflip(extra_mask) for extra_mask in extra_masks]
        ...
        if random.random() < self.rotate90_prob:
            ...
            if extra_masks is not None:
                extra_masks = [
                    TF.rotate(extra_mask, angle=angle, interpolation=InterpolationMode.NEAREST)
                    for extra_mask in extra_masks
                ]

        if random.random() < self.random_resized_crop_prob:
            pre, post, mask, extra_masks = self._safe_random_resized_crop(
                pre,
                post,
                mask,
            ...
            pre, post, mask, extra_masks = self._resize(pre, post, mask, extra_masks=extra_masks)

        if random.random() < self.color_jitter_prob:
            pre, post = self._apply_shared_color_jitter(pre, post)

        return pre, post, mask, extra_masks
    ...
    @staticmethod
    def _extra_masks_to_tensors(extra_masks: list[Image.Image] | None) -> list[torch.Tensor] | None:
        if extra_masks is None:
            return None
        return [
            torch.from_numpy(np.asarray(extra_mask, dtype=np.int64).copy())
            for extra_mask in extra_masks
        ]
    ...
    def make_dual_views(
        ...
        *,
        ...
        prepared_pre, prepared_post, prepared_mask, _ = self.prepare_augmented_pair(pre, post, mask)
    ...
    def make_dual_views_with_extra_masks(
        self,
        pre: Image.Image,
        post: Image.Image,
        mask: Image.Image,
        extra_masks: list[Image.Image],
        *,
        extra_context_prob: float = 0.0,
    ) -> tuple[
        tuple[torch.Tensor, torch.Tensor, torch.Tensor, dict[str, float]],
        ...
        prepared_pre, prepared_post, prepared_mask, prepared_extra_masks = self.prepare_augmented_pair(
            pre,
            post,
            mask,
            extra_masks=extra_masks,
        ...
        view1_extra_tensors = self._extra_masks_to_tensors(prepared_extra_masks)
        view2_extra_tensors = None if view1_extra_tensors is None else [tensor.clone() for tensor in view1_extra_tensors]
        return view1, view2, view1_extra_tensors, view2_extra_tensors
    ...
    def apply_with_extra_masks(
        ...
        *,
        ...
        pre_t, post_t, mask_t, context_stats = self.transform_prepared_pair(prepared_pre, prepared_post, prepared_mask)
        extra_tensors = self._extra_masks_to_tensors(prepared_extra_masks)
        if return_stats:
            return pre_t, post_t, mask_t, extra_tensors, context_stats
        return pre_t, post_t, mask_t, extra_tensors
...
class EvalPairTransform:
    ...
    def apply_with_extra_masks(
        ...
        extra_tensors = [
            torch.from_numpy(np.asarray(extra_mask, dtype=np.int64).copy())
            for extra_mask in resized_extra_masks
        ]
        return pre_t, post_t, mask_t, extra_tensors
