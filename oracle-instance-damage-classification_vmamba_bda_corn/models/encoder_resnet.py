...
def _load_resnet18(pretrained: bool) -> tuple[nn.Module, bool]:
    if ResNet18_Weights is not None:
        if pretrained:
            try:
                return resnet18(weights=ResNet18_Weights.IMAGENET1K_V1), True
            except Exception:
                return resnet18(weights=None), False
        return resnet18(weights=None), False
    return resnet18(pretrained=pretrained), bool(pretrained)
...
class ResNetFeatureEncoder(nn.Module):
    def __init__(self, backbone: str = "resnet18", in_channels: int = 4, pretrained: bool = True) -> None:
        ...
        backbone_model, loaded_pretrained = _load_resnet18(pretrained=pretrained)
        ...
        self.pretrained_load_summary = {
            ...
            "loaded": bool(loaded_pretrained),
            "status": "loaded_torchvision" if loaded_pretrained else ("disabled" if not pretrained else "fallback_random_init"),
            "source": "torchvision",
        }
