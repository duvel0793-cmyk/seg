...
from utils.official_bridge import (
    BRIDGE_MODE_CHOICES,
    BRIDGE_TARGET_SOURCE_CHOICES,
    BRIDGE_TIE_BREAK_CHOICES,
    run_official_bridge_evaluation,
)
...
def parse_args() -> argparse.Namespace:
    ...
    add_optional_bool_argument("--run_official_bridge_eval", default=None)
    add_optional_bool_argument("--save_bridge_instance_records", default=None)
    ...
    parser.add_argument("--bridge_mode", type=str, choices=BRIDGE_MODE_CHOICES, default=None)
    parser.add_argument("--bridge_target_source", type=str, choices=BRIDGE_TARGET_SOURCE_CHOICES, default=None)
    parser.add_argument("--bridge_output_dir", type=str, default=None)
    parser.add_argument("--bridge_split_prefix", type=str, default=None)
    parser.add_argument("--bridge_tie_break", type=str, choices=BRIDGE_TIE_BREAK_CHOICES, default=None)
    return parser.parse_args()


def ensure_training_defaults(config: dict[str, Any], default_loss_mode: str = "corn_adaptive_tau_safe") -> dict[str, Any]:
    model_cfg = config.setdefault("model", {})
    model_cfg.setdefault("model_type", "oracle_bda_vmamba_corn")
    model_cfg.setdefault("head_type", "corn")
    model_cfg.setdefault("backbone", "vmamba_tiny")
    model_cfg.setdefault("pretrained", True)
    model_cfg.setdefault("vmamba_pretrained_weight_path", "")
    model_cfg.setdefault("drop_path_rate", 0.1)
    model_cfg.setdefault("ambiguity_hidden_features", 256)
    model_cfg.setdefault("temporal_fusion_type", "bda_fusion")
    model_cfg.setdefault("temporal_reduction_ratio", 4)
    model_cfg.setdefault("use_abs_diff", True)
    model_cfg.setdefault("use_sum_feat", True)
    model_cfg.setdefault("use_prod_feat", False)
    model_cfg.setdefault("temporal_gate", True)
    model_cfg.setdefault("pool_modes", ["avg", "max", "attention"])
    ...
    eval_cfg.setdefault("run_official_bridge_eval", False)
    eval_cfg.setdefault("bridge_mode", "oracle_localization")
    eval_cfg.setdefault("bridge_target_source", "auto")
    eval_cfg.setdefault("bridge_output_dir", None)
    eval_cfg.setdefault("bridge_split_prefix", "test")
    eval_cfg.setdefault("bridge_tie_break", "deterministic_building_idx")
    eval_cfg.setdefault("save_bridge_instance_records", True)
    return config
...
def harmonize_model_and_loss_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if model_type not in {"oracle_mcd", "oracle_mcd_corn", "oracle_bda_vmamba_standard", "oracle_bda_vmamba_corn"}:
        raise ValueError(f"Unsupported model_type='{model_type}'.")

    if model_type in {"oracle_mcd_corn", "oracle_bda_vmamba_corn"} or head_type == "corn" or loss_mode in {"corn", "corn_adaptive_tau_safe"}:
        model_cfg["model_type"] = (
            "oracle_bda_vmamba_corn"
            if model_type.startswith("oracle_bda_vmamba")
            else "oracle_mcd_corn"
        )
...
def merge_config_dicts(base: dict[str, Any], override: dict[str, Any]) -> dict[str, Any]:
    """Recursively merge configs so local eval yaml can override checkpoint-bundled paths."""
...
def apply_overrides(
    ...
    for arg_name in [
        ...
        "run_official_bridge_eval",
        "save_bridge_instance_records",
    ]:
...
def resolve_checkpoint_path(config: dict[str, Any], explicit_checkpoint: str | None, resume_path: str | None = None) -> Path:
...
def unpack_model_outputs(model_output: Any) -> dict[str, Any]:
    if isinstance(model_output, torch.Tensor):
        return {
            ...
            "raw_delta_tau": None,
            "head_type": "standard",
            "encoder_feature_channels": None,
            "temporal_fusion_type": None,
            "pretrained_load_summary": None,
        }
    ...
    return {
        ...
        "encoder_feature_channels": model_output.get("encoder_feature_channels"),
        "temporal_fusion_type": model_output.get("temporal_fusion_type"),
        "pretrained_load_summary": model_output.get("pretrained_load_summary"),
    }
...
def write_tau_statistics(
    ...
    payload = {
        ...
        "temporal_fusion_type": config["model"].get("temporal_fusion_type", "n/a"),
        "vmamba_pretrained_weight_path": config["model"].get("vmamba_pretrained_weight_path", ""),
        "encoder_feature_channels": dict(getattr(getattr(model, "encoder", None), "feature_channels", {})),
    }
...
def write_analysis_report(
    ...
    if comparisons:
        for loss_mode in STANDARD_ORACLE_LOSS_MODES + ["corn", "corn_adaptive_tau_safe"]:
...
def main() -> None:
    ...
    requested_config = apply_overrides(load_config(args.config), args, default_loss_mode="corn_adaptive_tau_safe")
    requested_config["_config_path"] = str(Path(args.config).resolve())
    config = requested_config

    checkpoint_path = resolve_checkpoint_path(config, args.checkpoint, args.resume)
    ...
    if "config" in checkpoint:
        ...
        checkpoint_config = apply_overrides(
            ...
            args,
            default_loss_mode=checkpoint_default_loss_mode,
        )
        config = harmonize_model_and_loss_config(
            ensure_training_defaults(
                merge_config_dicts(checkpoint_config, requested_config),
                default_loss_mode=checkpoint_default_loss_mode,
            )
        )
    ...
    print(f"Backbone: {config['model'].get('backbone')}")
    print(f"Temporal fusion type: {config['model'].get('temporal_fusion_type', 'n/a')}")
    print(f"Head type: {config['model'].get('head_type', 'standard')}")
    print(f"Loss mode: {config['training']['loss_mode']}")
    print(f"Encoder feature channels: {dict(getattr(getattr(model, 'encoder', None), 'feature_channels', {}))}")
    print(f"VMamba pretrained weight path: {config['model'].get('vmamba_pretrained_weight_path', '')}")
    print(
        "Pretrained load summary: "
        f"{getattr(getattr(model, 'encoder', None), 'pretrained_load_summary', None)}"
    )
    ...
    for payload in [uncalibrated_metrics]:
        ...
        payload["backbone"] = config["model"].get("backbone")
        payload["temporal_fusion_type"] = config["model"].get("temporal_fusion_type", "n/a")
        payload["vmamba_pretrained_weight_path"] = config["model"].get("vmamba_pretrained_weight_path", "")
        payload["encoder_feature_channels"] = dict(getattr(getattr(model, "encoder", None), "feature_channels", {}))
    ...
    if calibration_payload is not None:
        ...
        calibrated_metrics["backbone"] = config["model"].get("backbone")
        calibrated_metrics["temporal_fusion_type"] = config["model"].get("temporal_fusion_type", "n/a")
        calibrated_metrics["vmamba_pretrained_weight_path"] = config["model"].get("vmamba_pretrained_weight_path", "")
        calibrated_metrics["encoder_feature_channels"] = dict(getattr(getattr(model, "encoder", None), "feature_channels", {}))
    ...
    metrics["backbone"] = config["model"].get("backbone")
    metrics["temporal_fusion_type"] = config["model"].get("temporal_fusion_type", "n/a")
    metrics["vmamba_pretrained_weight_path"] = config["model"].get("vmamba_pretrained_weight_path", "")
    metrics["encoder_feature_channels"] = dict(getattr(getattr(model, "encoder", None), "feature_channels", {}))
    ...
    tau_report = write_tau_statistics(eval_dir, inference_summary, ordinal_state, config, model)
    ...
    official_bridge_results = None
    if bool(config["evaluation"].get("run_official_bridge_eval", False)):
        official_bridge_results = run_official_bridge_evaluation(
            dataset=dataset,
            prediction_records=prediction_records,
            eval_dir=eval_dir,
            instance_metrics=metrics,
            bridge_mode=str(config["evaluation"].get("bridge_mode", "oracle_localization")),
            bridge_target_source=str(config["evaluation"].get("bridge_target_source", "auto")),
            bridge_output_dir=config["evaluation"].get("bridge_output_dir"),
            bridge_split_prefix=str(config["evaluation"].get("bridge_split_prefix", "test")),
            bridge_tie_break=str(config["evaluation"].get("bridge_tie_break", "deterministic_building_idx")),
            save_bridge_instance_records=bool(config["evaluation"].get("save_bridge_instance_records", True)),
        )
        metrics["official_bridge"] = {
            "enabled": True,
            "output_dir": official_bridge_results["output_dir"],
            "metrics": official_bridge_results["metrics"],
        }
    else:
        metrics["official_bridge"] = {"enabled": False}
    ...
    if official_bridge_results is not None:
        bridge_metrics = official_bridge_results["metrics"]
        print(
            f"official_bridge_score={bridge_metrics['score']:.4f} "
            f"official_bridge_localization_f1={bridge_metrics['localization_f1']:.4f} "
            f"official_bridge_damage_f1={bridge_metrics['damage_f1']:.4f}"
        )
        print(f"Official bridge artifacts saved to: {official_bridge_results['output_dir']}")
