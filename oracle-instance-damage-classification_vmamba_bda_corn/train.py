...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max_no_damage_train_samples", type=int, default=None)
    parser.add_argument("--cap_train_seed", type=int, default=None)
...
def ensure_training_defaults(config: dict[str, Any], default_loss_mode: str = "corn_adaptive_tau_safe") -> dict[str, Any]:
    ...
    train_cfg.setdefault("gradient_accumulation_steps", 1)
    ...
    train_cfg.setdefault("max_no_damage_train_samples", None)
    train_cfg.setdefault("cap_train_seed", int(config.get("seed", 3407)))
...
def load_and_override_config(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.gradient_accumulation_steps is not None:
        train_cfg["gradient_accumulation_steps"] = max(1, args.gradient_accumulation_steps)
    ...
    if args.max_no_damage_train_samples is not None:
        train_cfg["max_no_damage_train_samples"] = args.max_no_damage_train_samples
    if args.cap_train_seed is not None:
        train_cfg["cap_train_seed"] = args.cap_train_seed
...
def maybe_cap_no_damage_train_samples(
    train_dataset: XBDOracleInstanceDamageDataset,
    config: dict[str, Any],
) -> dict[str, Any] | None:
    """
    仅对 train_dataset 生效：
    - 保留全部 minor/major/destroyed
    - 仅把 no-damage 随机下采样到指定上限
    - 直接原地修改 train_dataset.samples 和 train_dataset.class_counts
    """
    train_cfg = config.get("training", {})
    cap = train_cfg.get("max_no_damage_train_samples", None)
    ...
    seed = int(train_cfg.get("cap_train_seed", config.get("seed", 3407)))
    rng = random.Random(seed)

    old_samples = train_dataset.samples
    old_class_counts = list(train_dataset.class_counts)

    no_damage_label = 0
    no_damage_indices = [
        idx for idx, sample in enumerate(old_samples)
        if int(sample["label"]) == no_damage_label
    ]

    if len(no_damage_indices) <= cap:
        summary = {
            "enabled": True,
            "seed": seed,
            "cap": cap,
            "old_total": len(old_samples),
            "new_total": len(old_samples),
            "old_no_damage": len(no_damage_indices),
            "new_no_damage": len(no_damage_indices),
            "old_class_counts": old_class_counts,
            "new_class_counts": old_class_counts,
            "message": "no-damage count already <= cap, nothing changed.",
        }
        train_dataset.no_damage_cap_summary = summary
        return summary

    kept_no_damage_indices = set(rng.sample(no_damage_indices, cap))

    new_samples: list[dict[str, Any]] = []
    for idx, sample in enumerate(old_samples):
        label = int(sample["label"])
        if label == no_damage_label and idx not in kept_no_damage_indices:
            continue
        new_samples.append(sample)

    new_class_counts = [0 for _ in old_class_counts]
    for sample in new_samples:
        new_class_counts[int(sample["label"])] += 1

    train_dataset.samples = new_samples
    train_dataset.class_counts = new_class_counts
    ...
    summary = {
        ...
        "old_no_damage": old_class_counts[0],
        "new_no_damage": new_class_counts[0],
        "old_class_counts": old_class_counts,
        "new_class_counts": new_class_counts,
        "message": "no-damage train samples were capped successfully.",
    }
    train_dataset.no_damage_cap_summary = summary
    return summary
...
def make_dataloaders(
    ...
    *,
    ...
    cap_summary = maybe_cap_no_damage_train_samples(train_dataset, config)
    if cap_summary is not None:
        print(
            "[Train no-damage cap] "
            f"seed={cap_summary['seed']} | "
            f"cap={cap_summary['cap']} | "
            f"total: {cap_summary['old_total']} -> {cap_summary['new_total']} | "
            f"no-damage: {cap_summary['old_no_damage']} -> {cap_summary['new_no_damage']}"
        )
        print(
            "[Train no-damage cap] "
            f"old class counts={cap_summary['old_class_counts']} | "
            f"new class counts={cap_summary['new_class_counts']}"
        )
...
def unpack_model_outputs(model_output: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "pre_features": model_output.get("pre_features"),
        "post_features": model_output.get("post_features"),
        "fused_features": model_output.get("fused_features"),
        "change_hints": model_output.get("change_hints"),
...
def maybe_print_vmamba_debug_shapes(model: torch.nn.Module, model_outputs: dict[str, Any]) -> None:
    if getattr(model, "_vmamba_debug_logged", False):
        return
    pre_features = model_outputs.get("pre_features")
    post_features = model_outputs.get("post_features")
    fused_features = model_outputs.get("fused_features")
    pooled_feature = model_outputs.get("pooled_feature")
    if not isinstance(pre_features, dict) or not isinstance(post_features, dict) or not isinstance(fused_features, dict):
        return

    print("VMamba first-batch feature shapes:")
    for name in ["c2", "c3", "c4", "c5"]:
        if name in pre_features:
            print(f"  pre_{name}: {tuple(pre_features[name].shape)}")
        if name in post_features:
            print(f"  post_{name}: {tuple(post_features[name].shape)}")
        if name in fused_features:
            print(f"  fused_{name}: {tuple(fused_features[name].shape)}")
    if pooled_feature is not None:
        print(f"  pooled_feature: {tuple(pooled_feature.shape)}")
    setattr(model, "_vmamba_debug_logged", True)
...
def run_epoch(
    ...
    debug_vmamba_shapes: bool = False,
) -> dict[str, Any]:
    ...
    for batch_index, batch in enumerate(progress, start=1):
        ...
        with context:
            ...
            if debug_vmamba_shapes:
                maybe_print_vmamba_debug_shapes(model, model_outputs)
        ...
        if is_train:
            ...
            scaled_loss_backward = loss_backward / float(accumulation_steps)
            if (
                ...
                head_only_soft_grads = torch.autograd.grad(
                    scaler.scale(loss_outputs["loss_safe_head_only"] / float(accumulation_steps)),
                    corn_head_parameters,
                    retain_graph=True,
                    allow_unused=True,
                )
            scaler.scale(scaled_loss_backward).backward()
            ...
            should_step = (batch_index % accumulation_steps == 0) or (batch_index == num_batches)
            if should_step:
...
def prepare_stage2_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    stage2_config["training"]["gradient_accumulation_steps"] = int(config["training"].get("gradient_accumulation_steps", 1))
    ...
    *,
    ...
    for epoch in range(total_epochs):
        ...
        train_metrics = run_epoch(
            ...
            gradient_accumulation_steps=int(stage2_config["training"].get("gradient_accumulation_steps", 1)),
        )
...
def _run_training(
    ...
    effective_batch_size = int(config["training"]["batch_size"]) * max(gradient_accumulation_steps, 1)
    ...
    print(
        "Pretrained loaded: "
        f"{bool(getattr(getattr(model, 'encoder', None), 'pretrained_load_summary', {}).get('loaded', False))}"
    )
    ...
    print(f"Stage2 enabled: {bool(config['training'].get('stage2_head_rebalance_enabled', False))}")
    ...
    for epoch in range(total_epochs):
        ...
        train_metrics = run_epoch(
            ...
            debug_vmamba_shapes="vmamba" in str(config["model"].get("backbone", "")),
        )
        val_metrics = run_epoch(
            model=model,
            loader=val_loader,
