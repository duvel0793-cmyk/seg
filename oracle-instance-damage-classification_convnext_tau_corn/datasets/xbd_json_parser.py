"""Robust xBD JSON parser with shapely-optional WKT handling."""
...
class XBDJsonParser:
    ...
    def parse_split(self, split: str) -> List[Dict]:
        ...
        for base_id in base_ids:
            instance_records = self._parse_base_id(base_id=base_id, label_dir=label_dir, image_dir=image_dir, split=split)
    ...
    def _parse_base_id(self, base_id: str, label_dir: Path, image_dir: Path, split: str) -> List[Dict]:
        ...
        for post_item in post_objects:
            ...
            polygon = post_item["polygon"] or (pre_item["polygon"] if pre_item else None)
            if not polygon:
                continue
    ...
    def _load_buildings(self, json_path: Path) -> List[Dict]:
        ...
        for item in xy_features:
            ...
            polygon = self._parse_wkt(item.get("wkt", ""))
