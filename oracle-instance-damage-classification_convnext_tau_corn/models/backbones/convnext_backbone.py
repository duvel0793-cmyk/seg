...
class ConvNeXtBackbone(nn.Module):
    ...
    def load_pretrained(self, pretrained_path: str) -> None:
        ...
        for candidate_key in ("model", "state_dict", "model_state", "backbone"):
            if isinstance(checkpoint, dict) and candidate_key in checkpoint:
                state_dict = checkpoint[candidate_key]
                break
        if not isinstance(state_dict, dict):
            raise TypeError(f"Unsupported checkpoint format at '{pretrained_path}'.")
        ...
        for key, value in state_dict.items():
            ...
            if clean_key in {"head.weight", "head.bias"}:
                continue
            cleaned_state_dict[clean_key] = value

        incompatible = self.backbone.load_state_dict(cleaned_state_dict, strict=False)
        missing = list(incompatible.missing_keys)
        unexpected = list(incompatible.unexpected_keys)
        print(f"[ConvNeXtBackbone] loaded checkpoint: {pretrained_path}")
        print(f"[ConvNeXtBackbone] missing keys: {len(missing)}")
        print(f"[ConvNeXtBackbone] unexpected keys: {len(unexpected)}")
