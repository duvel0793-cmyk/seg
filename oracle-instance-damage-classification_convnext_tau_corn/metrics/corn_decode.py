...
VALID_DECODE_MODES = {"probability_argmax", "threshold_count"}
...
def decode_corn_logits(logits: torch.Tensor, mode: str = "probability_argmax") -> torch.Tensor:
    """Decode CORN logits into ordinal class predictions."""
    if mode not in VALID_DECODE_MODES:
        raise ValueError(f"Unsupported CORN decode mode: {mode}")
