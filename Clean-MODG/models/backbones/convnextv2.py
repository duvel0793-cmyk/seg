...
def _is_convnextv2_candidate(candidate: str) -> bool:
    return "convnextv2" in candidate


def _is_convnextv2_checkpoint(checkpoint_path: str | Path | None) -> bool:
    if checkpoint_path is None:
        return False
    path = Path(checkpoint_path)
    name = path.name.lower()
    return "convnextv2" in name or "fcmae" in name
...
class TimmBackbone(nn.Module):
    def __init__(self, name: str, pretrained: bool = True, in_chans: int = 3, checkpoint_path: str | Path | None = None) -> None:
        ...
        self._warned_checkpoint_skip = False
        ...
        for candidate in _candidate_names(name):
            try:
                ...
                if pretrained and self.checkpoint_path is not None:
                    if not _is_convnextv2_checkpoint(self.checkpoint_path):
                        pretrained_cfg_overlay = {"file": str(self.checkpoint_path)}
                    elif _is_convnextv2_candidate(candidate):
                        pretrained_cfg_overlay = {"file": str(self.checkpoint_path)}
                    elif not self._warned_checkpoint_skip:
                        warnings.warn(
                            f"Skipping ConvNeXtV2 checkpoint '{self.checkpoint_path}' for fallback backbone "
                            f"candidate '{candidate}'. The fallback backbone will use its own timm weights instead.",
                            stacklevel=2,
                        )
                        self._warned_checkpoint_skip = True
                ...
                if candidate != name:
                    fallback_note = ""
                    if self.checkpoint_path is not None and _is_convnextv2_checkpoint(self.checkpoint_path) and not _is_convnextv2_candidate(candidate):
                        fallback_note = " Local ConvNeXtV2 checkpoint loading was disabled for this fallback candidate."
                    warnings.warn(
                        f"Backbone '{name}' is unavailable. Falling back to '{candidate}'.{fallback_note}",
                        stacklevel=2,
                    )
                break
