"""Test entrypoint for Clean-MODG."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run test inference for Clean-MODG.")
...
def main() -> None:
    ...
    logger = setup_logger(dirs["root"], name="Clean-MODG-test")
    device = get_device()
    split = config["data"].get("split_test", "test")
    loader = build_eval_loader(config, split=split)
    ...
    logger.info("Test | %s", format_metrics_line(metrics))
    logger.info("Per-class F1: %s", [round(x, 4) for x in metrics["per_class_f1"]])
    logger.info("Class order: %s", CLASS_NAMES)
    save_predictions_csv(predictions, dirs["predictions"] / "test_predictions.csv")
