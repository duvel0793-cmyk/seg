...
def image_to_tensor(image: np.ndarray) -> torch.Tensor:
    ...
    should_scale_255 = False
    if np.issubdtype(original_dtype, np.integer):
        should_scale_255 = True
    elif image_min >= 0.0 and image_max > 5.0:
        # Only rescale float inputs when they clearly still live in a raw 0-255 style range.
        # Normalized RGB tensors can be > 1 while remaining semantically correct, especially after
        # ImageNet normalization or RGBM concatenation, so we keep the threshold intentionally loose.
        should_scale_255 = True
