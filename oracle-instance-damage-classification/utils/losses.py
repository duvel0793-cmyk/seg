...
class AdaptiveUCLCDALoss(LearnableSeverityAxis):
    ...
    def _compute_outputs(
        ...
        *,
        ...
        resolved_loss_tau_reg = resolved_loss_tau_mean + resolved_loss_tau_diff
...
class AdaptiveUCLCDALossV3(AdaptiveUCLCDALoss):
    ...
    def forward(
        ...
        tau_mean = tau.float().mean() if tau.numel() > 0 else tau.new_tensor(self.tau_target).float()
        loss_tau_mean = (tau_mean - self.tau_target).pow(2).to(device=tau.device, dtype=tau.dtype)
...
class DamageLossModule(nn.Module):
    def __init__(
        ...
        lambda_tau_mean: float = 0.005,
        lambda_tau_diff: float = 0.03,
            ...
            if self.loss_mode in {
                ...
                if self.loss_mode in {"adaptive_ucl_cda", "adaptive_ucl_cda_v2", "adaptive_ucl_cda_v3"}:
                    ...
                    if self.loss_mode == "adaptive_ucl_cda_v2":
                        ...
                        total_loss = total_loss + self.lambda_tau_mean * loss_tau_mean + self.lambda_tau_diff * loss_tau_diff
                else:
                    ordinal_outputs = self.ordinal_loss(logits, targets)
