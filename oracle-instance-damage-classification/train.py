...
def resolve_tau_phase(config: dict[str, Any], epoch_index: int) -> dict[str, Any]:
    ...
    freeze_epochs = int(config["training"].get("tau_freeze_epochs", 5 if loss_mode == "adaptive_ucl_cda_v3" else 3))
...
def run_epoch(
    ...
    for batch in progress:
        ...
        if tau_used is not None:
            tau_values.extend([float(value) for value in tau_used.detach().float().cpu().tolist()])
