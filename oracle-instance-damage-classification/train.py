    ...
    for batch in progress:
        ...
        if is_train:
            ...
            if (
                ...
                and float(getattr(criterion, "lambda_corn_soft", 0.0)) > 0.0
            ):
                head_only_soft_grads = torch.autograd.grad(
                    scaler.scale(criterion.lambda_corn_soft * loss_outputs["loss_corn_soft"]),
        ...
        if loss_outputs["class_probabilities"] is not None:
            ...
            corn_entropy = -(class_probabilities.clamp_min(1e-8) * class_probabilities.clamp_min(1e-8).log()).sum(dim=1)
